...
bool CDevice::getDeviceCommunicationFault()
{
    ...
    CDistribution* distribution = controller->distributionByAddress(deviceValue(DEVICE_VALUE_DISTRIBUTIONADDR).toInt());
    ...
    if(distribution->distributionValue(OBJECT_VALUE_ONLINE) == 0 ||  loop->loopValue(OBJECT_VALUE_ONLINE) == 0
            || loop->loopValue(LOOP_VALUE_NOLOGIN) == 1 || deviceValue(OBJECT_VALUE_ONLINE) == 0 ||
