...
void CDesignSplashView::setPixmap(QPixmap background)
{
    ...
    // 获取label尺寸
    double labelWidth = m_pView->width();
    double labelHeight = m_pView->height();
...
//    painter.scale(startHeight/40, startHeight/40);
    ...
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            ...
            else if(distance == minDistance && j<=3)
            {
                p1 = startPoint[i];
                p2 = endPoint[j];
            }
...
//    else if(joint == 1)
//        pointRotationAngle = std::atan2(p1.y() - p2.y(), p1.x() - p2.x()) * 180 / M_PI;


    //    bool isRight = false;  //默认向左或者垂直向上
    //    //向右或者垂直向下
    ...
    QPointF line[5];
    double line_compare1 = 1*m_changeValue/100.0;
    double line_compare2 = 2.5*m_changeValue/100.0;
    if(joint == 2)
    {
        QPointF arrowPoints[5] = {
            QPointF(p1.x() - line_compare1 + minDistance, p1.y() + 3*line_compare2),
            QPointF(p1.x() + line_compare1, p1.y() + 3*line_compare2),
            QPointF(p1.x() + line_compare1, p1.y() + line_compare2),
            QPointF(p1.x() - line_compare1 + minDistance, p1.y() + line_compare2),
            QPointF(p1.x() - line_compare1 + minDistance, p1.y() + 3*line_compare2)
        };
        QPointF p1_compare(p1.x(),p1.y()+2*line_compare2);
        for (int i = 0; i < 5; ++i) {
            line[i] = rotatePoint(arrowPoints[i], p1_compare, pointRotationAngle);
        }
    }
    else if(joint == 1)
    {
...
//        painter.setBrush(QBrush(QColor(255,255,255)));  // 设置填充颜色为透明
    ...
    //判断箭头方向
    QPointF arrow[5];
    double arrow_compare1 = 1*m_changeValue/100.0;
    double arrow_compare2 = 2.5*m_changeValue/100.0;
    double arrow_compare3 = 10*m_changeValue/100.0;
    double arrow_compare4 = 5*m_changeValue/100.0;
    if(joint == 2)
//    if(!isRight)
    {
        QPointF arrowPoints[5] = {
            QPointF(p1.x() - arrow_compare1 + minDistance, p1.y() + 3*arrow_compare2),
            QPointF(p1.x() - arrow_compare1 + minDistance - std::sqrt(3.0) * (arrow_compare3 + arrow_compare4 * std::sqrt(3.0)) * 0.5,
                   p1.y() + 3*arrow_compare2 + (arrow_compare3 + arrow_compare4 * std::sqrt(3.0)) * 0.5),
            QPointF(p1.x() - arrow_compare1 + minDistance - std::sqrt(3.0) * arrow_compare3 * 0.5 - arrow_compare3,
                   p1.y() + 3*arrow_compare2 + arrow_compare3 * 0.5),
            QPointF(p1.x() - arrow_compare1 + minDistance - arrow_compare3, p1.y() + 3*arrow_compare2),
            QPointF(p1.x() - arrow_compare1 + minDistance, p1.y() + 3*arrow_compare2)
        };
        QPointF p1_compare(p1.x(),p1.y()+2*line_compare2);
        for (int i = 0; i < 5; ++i) {
            arrow[i] = rotatePoint(arrowPoints[i], p1_compare, pointRotationAngle);
        }
//        QPointF arrowPoints[5] = {
//            QPointF(p1.x() - 1 + minDistance, p1.y() + 2.5),
...
//    else
    {
        QPointF arrowPoints[5] = {
            QPointF(p1.x() - arrow_compare1 + minDistance, p1.y() - arrow_compare2),
            QPointF(p1.x() - arrow_compare1 + minDistance - std::sqrt(3.0) * (arrow_compare3 + arrow_compare4 * std::sqrt(3.0)) * 0.5,
                   p1.y() - arrow_compare2 + (arrow_compare3 + arrow_compare4 * std::sqrt(3.0)) * 0.5),
            QPointF(p1.x() - arrow_compare1 + minDistance - std::sqrt(3.0) * arrow_compare3 * 0.5 - arrow_compare3,
                   p1.y() - arrow_compare2 + arrow_compare3 * 0.5),
            QPointF(p1.x() - arrow_compare1 + minDistance - arrow_compare3, p1.y() - arrow_compare2),
            QPointF(p1.x() - arrow_compare1 + minDistance, p1.y() - arrow_compare2)
        };
        QPointF p1_compare(p1.x(),p1.y()-2*line_compare2);
        for (int i = 0; i < 5; ++i) {
