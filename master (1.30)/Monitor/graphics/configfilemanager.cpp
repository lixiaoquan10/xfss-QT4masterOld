...
void ConfigFileManager::ParseBRANCH_LEVEL1(QDomNode *node)//解析一级分支
{
    ...
    while(!childnode.isNull())
    {
        if(childnode.toElement().tagName() == "Controller")
        {
            ...
            if(!controller)
                return;
            controller->setControllerValue(CONTROLLER_VALUE_NAME,childnode.toElement().attribute("name"));
            controller->setControllerValue(CONTROLLER_VALUE_ADDRESS,childnode.toElement().attribute("address").toInt());
