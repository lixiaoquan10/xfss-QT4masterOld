...
void dlgUseTime::on_m_btnOk_clicked()
{
    ...
    if(bVerified)
    {
        ...
        CGlobal::instance()->TimeInterval()->m_LinkmanPhoneNumer = ui->m_linkmanPhoneNumber->text();
        ...
        query.exec(QString("INSERT OR REPLACE INTO ParamInfo (ParamName, ParamValue) VALUES ('LinkmanPhoneNumber', '%1');").arg(CGlobal::instance()->TimeInterval()->m_LinkmanPhoneNumer));
        db.commit();
        db.close();
        //跳转到usetime界面
        CGlobal::instance()->centerFrameContainer()->setCurrentIndex(5);
        // 设置窗口为非模态对话框
        CGlobal::instance()->TimeInterval()->setModal(false);
        // 将 TimeInterval 界面设为可用
        CGlobal::instance()->TimeInterval()->setEnabled(true);
        this->close();
    }
}
...
bool dlgUseTime::VerifyPowerCode(QString strPowerCode, int nLocalNumber, int &nDayCounts)
{
    ...
    if (nSum != nVerifySum)
    {
        QMessageBox::information(this, "使用期限", "权限序号错误");
        return false;
    }
    ...
    if (nSum != nVerifySum)
    {
        QMessageBox::information(this, "使用期限", "本机序号错误");
        return false;
    }
