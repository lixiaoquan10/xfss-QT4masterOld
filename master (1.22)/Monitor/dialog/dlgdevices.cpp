...
void dlgDevices::getDeviceInfo()
{
    ...
    int maxPage = controller->getDeviceTotal()%13 > 0 ? (controller->getDeviceTotal()/13+1) : (controller->getDeviceTotal()/13);
...
//                                m_model->addData(tempInfo);
                                if(deviceCount == currentDeviceMaxNumber || deviceCount == controller->getDeviceTotal() )
                                {
                                    m_model->clear();
