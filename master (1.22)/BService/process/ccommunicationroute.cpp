...
//    createCommunication(CT_PowerCard);
...
void CCommunicationRoute::initCommunicationManager()
{
    ...
    connect(this, SIGNAL(sendPrintData(QString,int,QHash<QString,QVariant>,QByteArray)), m_communicationManager,
            SLOT(sendPrintData(QString,int,QHash<QString,QVariant>,QByteArray)), Qt::DirectConnection);
}
...
void CCommunicationRoute::setLedStatus(const int ledStatus1, const int ledStatus2, const int ledStatus3)
{
    ...
    if(communicationIsActive(communication))
        emit sendPrintData(communication, SetLedStatus, control, data);
}
