...
public:
    ...
    CLayer(CProject *project, CLayer* parent = 0, const int keyId = 0);
    ~CLayer();

    static int loadLayer(CDBSqlite *db, CProject* project);
    ...
    bool setLayerId(int id) { return setValue(LAYER_VALUE_ID, id); }
    int layerId() const { return value(LAYER_VALUE_ID).toInt(); }
