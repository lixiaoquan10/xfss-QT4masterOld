...
//    CObject* data = getObject(index);
    ...
    if(role== Qt::DisplayRole){
        ...
        else if(index.column() == DEVICE_COLUMN_STATUS)
        {
            if(device->deviceValue(DEVICE_VALUE_EMERGENCY) == "应急")
                ...
                return device->getStatusDes();
        }
    }
    if(role == Qt::BackgroundColorRole){
        if(index.column() == DEVICE_COLUMN_STATUS)
        {
            if(device->getStatus(OBJS_LightFault) == 1 || device->getStatus(OBJS_LightCommunicationFault) == 1)
                return QColor(245,222,179); //黄色
            else if(device->getStatus(OBJS_ManualEmergency) == 1 || device->getStatus(OBJS_ForceEmergency) == 1 ||
                    device->getStatus(OBJS_DIEmergency) == 1 || device->getStatus(OBJS_TestEmergency) == 1 ||
                    device->getStatus(OBJS_SystemEmergency) == 1 || device->getStatus(OBJS_UndervoltageEmergency) == 1 ||
                    device->getStatus(OBJS_AutoEmergency) == 1 || device->deviceValue(DEVICE_VALUE_EMERGENCY) == "应急" ||
                    device->deviceValue(DEVICE_VALUE_TEST) == "测试")
                return QColor(255,99,71);  //红色
            else if(device->getStatusDes() == "正常")
    ...
    if(role == Qt::TextColorRole){
        if(device->getCommunicationStatus() == "正常")
            return QColor(0,0,0);
        else
