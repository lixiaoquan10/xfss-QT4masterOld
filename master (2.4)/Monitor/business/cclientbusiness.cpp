                    ...
                    if(distribution)
                    {
                        ...
                        for(int i=1; i<9; i++){
                            ...
                            if(!loop)
                            {
                                ...
                                loop->setloopValue(LOOP_VALUE_CANPORTADDR,i);
                            }
                        }
...
//                                        serverDeviceRealtimeData(distribution);
                ...
                if(distribution)
                {
                    ...
                    if(loop && msgObjectStatus->strText != ""){
                        ...
                        {
                            if(device->deviceValue(OBJECT_VALUE_ONLINE) == 0 || msgObjectStatus->nCommunicationStatus == OBJS_Online)
                            {
                                device->setDeviceValue(OBJECT_VALUE_ONLINE,1);
                            }
                            ...
                            device->setDeviceValue(DEVICE_VALUE_TYPE_ID, msgObjectStatus->nValue);
                            device->setDeviceValue(DEVICE_VALUE_ADDR, msgObjectStatus->nDeviceID);
                            device->setDeviceValue(DEVICE_VALUE_CHANGEADD,0);
...
//                                    status = 1;
                                    ...
                                    for(int i=0; i<loops.count(); i++){
                                        ...
                                        if(loop && !loop->getStatus(OBJS_LoopCommunicationFault) &&
                                                loop->loopValue(OBJECT_VALUE_ONLINE) == 1 && loop->loopValue(LOOP_VALUE_NOLOGIN) == 0)
                                        {
...
void CClientBusiness::Emergency()
{
    ...
    if(!db.open())
        return;
    //闪烁(所有灯）
    QSqlQuery query = db.query(QString("SELECT l.Remark,l.Address,c.Loop,a.CanPort,a.Address,r.Number from LampInfo l "
    ...
    while(query.next())
    {
        CCanport* canport = CGlobal::instance()->controller()->canportByAddress(query.value(3).toInt()+2);
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(query.value(4).toInt());
        ...
        if(!loop || loop->getStatus(OBJS_LoopCommunicationFault) || loop->loopValue(LOOP_VALUE_NOLOGIN) == 1)
            continue;
...
//        device->setDeviceValue(DEVICE_VALUE_EMERGENCY,"应急");
        ...
        uploadLampStatus(device);
        direction = 0x07;
        if(CGlobal::instance()->programDeviceView()->type(query.value(5).toInt()) == NCT_LightType)
            continue;
        if(((!isUCLamp(query.value(5).toInt())) && (CGlobal::instance()->m_SendDirectionAndTwinkleByOne == true))
                || (isUCLamp(query.value(5).toInt()) && (CGlobal::instance()->m_UCSendDirectionAndTwinkleByOne == true)))
        {
            direction |= 0x08;
        }
        else
    ...
    //方向控制(双向灯具或地埋灯)
    QSqlQuery query1 = db.query(QString("SELECT l.LampDirection,r.Remark,r.Address,c.Loop,a.CanPort,a.Address,r.id,r.LampTypeID,r.IsReverse from EvacuationPathItem l "
    ...
    while(query1.next())
    {
        CCanport* canport = CGlobal::instance()->controller()->canportByAddress(query1.value(4).toInt()+2);
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(query1.value(5).toInt());
        if(!distribution || distribution->getStatus(OBJS_DistributionCommunicationFault))
            continue;
        CLoop* loop = distribution->loopByAdd(query1.value(3).toInt());
        if(!loop || loop->getStatus(OBJS_LoopCommunicationFault) || loop->loopValue(LOOP_VALUE_NOLOGIN) == 1)
            continue;
        CDevice* device = loop->deviceByAdd(query1.value(2).toInt());
        if(!device || device->getStatus(OBJS_LightCommunicationFault))
            continue;
        ...
        if(((!isUCLamp(query1.value(7).toInt())) && (CGlobal::instance()->m_SendDirectionAndTwinkleByOne == true))
                || (isUCLamp(query1.value(7).toInt()) && (CGlobal::instance()->m_UCSendDirectionAndTwinkleByOne == true)))
        {
            direction |= 0x08;
        ...
        msgObjectStatus.nValue = direction;
        //上传灯具指向到云平台
        XmlUploadLampDirection(query1.value(6).toInt(),directionStatus);
        //服务端上传灯具指向
        serverLampDirectionUpload(query1.value(6).toInt(),directionStatus);
        exeCommand(NCT_DeviceDirection, &msgObjectStatus);
    ...
    //可变状态灯
    QSqlQuery query2 = db.query(QString("SELECT l.Remark,l.Address,c.Loop,a.CanPort,a.Address from LampInfo l "
                                       " left join ChannelInfo c on l.ChannelInfoID = c.ID "
                                       " left join CanDeviceInfo a on c.CanDeviceInfoID = a.ID "
    ...
    while(query2.next())
    {
        CCanport* canport = CGlobal::instance()->controller()->canportByAddress(query2.value(3).toInt()+2);
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(query2.value(4).toInt());
        if(!distribution || distribution->getStatus(OBJS_DistributionCommunicationFault))
            continue;
        CLoop* loop = distribution->loopByAdd(query2.value(2).toInt());
        if(!loop || loop->getStatus(OBJS_LoopCommunicationFault) || loop->loopValue(LOOP_VALUE_NOLOGIN) == 1)
            continue;
        CDevice* device = loop->deviceByAdd(query2.value(1).toInt());
        if(!device || device->getStatus(OBJS_LightCommunicationFault))
            continue;
        CMsgObjectStatus msgObjectStatus;
        msgObjectStatus.nCanportAdd = canport->canportAdd();
...
void CClientBusiness::objectStatusChanged(CObject* object, short status, unsigned char oldValue, unsigned char newValue, unsigned int time)
{
    ...
    if(objectType != CCanport::type_s && objectType != CDistribution::type_s  &&
    ...
    if(objectType == CCanport::type_s)
    {
        ...
        if(canport && canport->canportId() == m_canportInfo.address)
            canportStatusChanged(canport, status, newValue);
        ...
        m_statusInfo.area = canport->canportValue(DEVICE_VALUE_AREA).toString();
        m_statusInfo.location = canport->canportValue(DEVICE_VALUE_LOCATION).toString();
    ...
    else if(objectType == CDistribution::type_s)
    {
        ...
        m_msgLog->Canport = 1;
        m_msgLog->CanDeviceAddress = distribution->distributionValue(DISTRIBUTION_VALUE_ADDR).toInt();
        m_msgLog->Loop = 0;
        m_msgLog->LampDeviceAddress = 0;
        m_msgLog->title = strtext;
        exeCommand(NCT_AddLog, m_msgLog);

        m_statusInfo.canID = m_msgLog->CanDeviceAddress;
        m_statusInfo.loopID = 0;
        m_statusInfo.deviceID = 0;
    ...
    else if(objectType == CLoop::type_s)
    {
        ...
        m_msgLog->Loop = loop->loopValue(LOOP_VALUE_ADDR).toInt();
        m_msgLog->LampDeviceAddress = 0;
        m_msgLog->title = strtext;
        if(type != SRT_Emergency)
            exeCommand(NCT_AddLog, m_msgLog);
...
//                statusInfo.description = device->descriptionStr();
                ...
                statusInfo.layer = "";
                statusInfo.canID = device->deviceValue(DEVICE_VALUE_DISTRIBUTIONADDR).toInt();
...
void CClientBusiness::canportStatusChanged(CCanport *canport, short status, unsigned char value)
{
    QList<QVariant> param;
    ...
    if(standbyFault != hasStandbyPowerFault(canport))
    {
        standbyFault = hasStandbyPowerFault(canport);
        param.clear();
