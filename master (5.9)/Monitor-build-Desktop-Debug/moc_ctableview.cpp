/****************************************************************************
** Meta object code from reading C++ file 'ctableview.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../Monitor/view/ctableview.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'ctableview.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_CTableView[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: signature, parameters, type, tag, flags
      29,   12,   11,   11, 0x05,
      66,   62,   11,   11, 0x05,

 // slots: signature, parameters, type, tag, flags
      89,   82,   11,   11, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_CTableView[] = {
    "CTableView\0\0supportedActions\0"
    "signalStartDrag(Qt::DropActions)\0row\0"
    "rowChanged(int)\0editor\0commitData(QWidget*)\0"
};

void CTableView::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        CTableView *_t = static_cast<CTableView *>(_o);
        switch (_id) {
        case 0: _t->signalStartDrag((*reinterpret_cast< Qt::DropActions(*)>(_a[1]))); break;
        case 1: _t->rowChanged((*reinterpret_cast< const int(*)>(_a[1]))); break;
        case 2: _t->commitData((*reinterpret_cast< QWidget*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData CTableView::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject CTableView::staticMetaObject = {
    { &QTableView::staticMetaObject, qt_meta_stringdata_CTableView,
      qt_meta_data_CTableView, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &CTableView::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *CTableView::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *CTableView::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_CTableView))
        return static_cast<void*>(const_cast< CTableView*>(this));
    return QTableView::qt_metacast(_clname);
}

int CTableView::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QTableView::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void CTableView::signalStartDrag(Qt::DropActions _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void CTableView::rowChanged(const int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
