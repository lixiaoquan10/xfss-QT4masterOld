...
//        device->setDeviceValue(DEVICE_VALUE_EMERGENCY,"应急");
    ...
    while(query1.next())
    {
        int lampDirection = query1.value(0).toInt();
        int canAddress = query1.value(4).toInt()+2;
        int distributionAddress = query1.value(5).toInt();
        int loopAddress = query1.value(3).toInt();
        int deviceAddress = query1.value(2).toInt();
        int lampID = query1.value(6).toInt();
        int lamptypeID = query1.value(7).toInt();
        int lampIsReverse = query1.value(8).toInt();
        CCanport* canport = CGlobal::instance()->controller()->canportByAddress(canAddress);
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(distributionAddress);
...
//火警点协议联动应急
void CClientBusiness::firePointEmergency(int firePointID)
{
    int direction = 0;
    ...
    QSqlQuery query = db.query(QString("SELECT l.Address,c.Loop,a.CanPort,a.Address,r.Number from LampInfo l "
                                       ...
                                       " left join LampToFirePoint f on f.LampInfoID=l.ID "
    ...
    QSqlQuery query1 = db.query(QString("SELECT l.LampDirection,r.Remark,r.Address,c.Loop,a.CanPort,a.Address,r.id,r.LampTypeID,r.IsReverse from EvacuationPathItem l "
                                       ...
                                       " left join ChannelInfo c on r.ChannelInfoID = c.ID "
                                       " left join CanDeviceInfo a on c.CanDeviceInfoID = a.ID "
                                       " where l.FirePointID = %1").arg(firePointID));
    //query.value(0)代表第0列
    //第0列:l.LampDirection  灯具方向
    //第1列:r.Remark         灯具产品编号
    //第2列:r.Address        灯具地址
    ...
    while(query1.next())
    {
        int lampDirection = query.value(0).toInt();
        int canAddress = query.value(4).toInt()+2;
        int distributionAddress = query.value(5).toInt();
        int loopAddress = query.value(3).toInt();
        int deviceAddress = query.value(2).toInt();
        int lampID = query.value(6).toInt();
        int lamptypeID = query.value(7).toInt();
        int lampIsReverse = query.value(8).toInt();
        CCanport* canport = CGlobal::instance()->controller()->canportByAddress(canAddress);
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(distributionAddress);
