...
void CDesignSplashView::setPrimitiveItem(int layerID)
{
    ...
    while(querylamp.next())
    {
        ...
        CCanport* canport = CGlobal::instance()->controller()->canportByAddress(querylamp.value(10).toInt()+2);
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(querylamp.value(11).toInt());
        if(!distribution)
            continue;
        CLoop* loop = distribution->loopByAdd(querylamp.value(9).toInt());
        if(!loop)
            continue;
        CDevice* device = loop->deviceByAdd(querylamp.value(8).toInt());
        if(!device)
            continue;
//        int LampInfoID = query.value(fieldLampInfoID).toInt();
    ...
    if(CGlobal::instance()->m_isEmergency)
    {
        ...
        {
            CCanport* canport1 = CGlobal::instance()->controller()->canportByAddress(queryConnection.value(6).toInt()+2);
            if(!canport1)
                continue;
            ...
            CCanport* canport2 = CGlobal::instance()->controller()->canportByAddress(queryConnection.value(10).toInt()+2);
            if(!canport2)
                continue;
...
void CDesignSplashView::setFirePointPrimitiveItem(QPixmap* background,CFirePoint* firepoint)
{
    ...
    double x = firepoint->firePointValue(FIREPOINT_VALUE_CX).toDouble();
    double y = firepoint->firePointValue(FIREPOINT_VALUE_CY).toDouble();
    double width = firepoint->firePointValue(FIREPOINT_VALUE_WIDTH).toDouble();
    double height = firepoint->firePointValue(FIREPOINT_VALUE_HEIGHT).toDouble();
