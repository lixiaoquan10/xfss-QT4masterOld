...
{
    qDebug() << "CLog::addLog" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    int64_t count;
...
{
    qDebug() << "CLog::getLog" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    QList<CMsgLog*> list;
...
bool CLog::createDB()
{
    qDebug() << "CLog::createDB" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    QSqlQuery query(m_db);
    if (!query.exec(
...
{
    qDebug() << "CLog::checkDB" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
...
{
    qDebug() << "CLog::backupDB" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    QFile file(dbName);
...
{
    qDebug() << "CLog::openDB" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    m_dbName = dbName;
...
void CLog::closeDB()
{
    qDebug() << "CLog::closeDB" << QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    if(m_db.isOpen())
        m_db.close();
    m_db = QSqlDatabase();
