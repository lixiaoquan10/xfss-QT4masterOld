        ...
        foreach (int infoType, m_hashCommunicationLine[linename]->sendDataTableAsType.keys()){
            ...
            if(sendDataList.count() > 0){
                ...
                if(!CGlobal::instance()->m_canport1ID)
                {
                    ...
                    {
                        if(i<2)
                            packetTemplate.setCanId(i+3);//3 4
                        else
                            packetTemplate.setCanId(i+5);//7 8
        ...
        foreach (int infoType, m_hashCommunicationLine[linename]->sendDataTableAsType.keys()){
            ...
            if(sendDataList.count() > 0){
                ...
                if(!CGlobal::instance()->m_canport2ID)
                {
                    ...
                    {
                        if(i<2)
                            packetTemplate.setCanId(i+5);//5  6
                        else
                            packetTemplate.setCanId(i+7);//9  10
                        const QByteArray& byteArray = packetTemplate.buildBytes(&nSerialNumber);
...
//        QString data = "handleLedReciveData:  " + led_reciveData.toHex() + " " + QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz") + "\n";
