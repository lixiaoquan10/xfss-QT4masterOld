    ...
    switch(type)
    {
        case NCT_ObjectStatus:
        {
            ...
            if(msgObjectStatus->nType == 0x01){
                ...
                CCanport* canport = CGlobal::instance()->controller()->canportByAddress(msgObjectStatus->nCanportID);
                if(!canport)
                    canport = createUndefinedCanport(CGlobal::instance()->controller(), msgObjectStatus->nCanportID);
