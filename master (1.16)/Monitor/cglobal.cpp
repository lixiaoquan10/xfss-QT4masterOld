...
//#include "widget/cdesignmainview.h"
...
CProject* CGlobal::project()
{
    if(m_dm)
        return m_dm->projectAt(0);
    else
        return 0;
}

CController* CGlobal::panel()
{
    CProject *p = project();
    if(p && p->hasController(m_nPanelAddress))
        return p->controllerById(m_nPanelAddress);
    else
        return 0;
}
...
void CGlobal::saveSetting()
{
    ...
    settingcfg.setValue("projectname", m_projectName);
...
{
    int keyId;
    if(type == OBJT_Project)
    {
        keyId = Id;
    }
    else if(type == OBJT_Controller)
    {
        keyId = Id;
    }
...
void CGlobal::readSetting()
{
    ...
    m_projectName = settingcfg.value("projectname").toString();
...
const QList<CLayer *> CGlobal::getLayers(const QList<CProject*> &projectList)
{
    QList<CLayer *> layerList;
    for(int i=0; i<projectList.count(); ++i)
    {
        QList<CLayer *> tempLayerList = projectList.at(i)->layers();//??????????
