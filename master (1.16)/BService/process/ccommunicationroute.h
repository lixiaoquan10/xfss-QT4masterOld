...
public:
    ...
    void setFasConfig(const int port, const int commpnyid, const int npanel);
    void setPortSpeed(const QString linename, const int port, const int speed);
    int  getPortSpeed(const QString linename, const int port) const;
    void setLedStatus(const int ledStatus1, const int ledStatus2, const int ledStatus3);
    void getInputPortStatus();
    void setOutputPortStatus(const bool enable);
    void setPowerStatus(const bool enable);
    void printData(const QByteArray& data);
    void reset(bool binit = false);
    void resetPower();
    void resetLed(bool binit);
    void resetFas();
    void emergency(bool enable);
...
//    void initLineInfo();
    ...
    bool setPortSpeedValue(const QString linename, const int port, const int speed);
    int  getPortSpeedValue(const QString linename, const int port) const;
    ...
    void procInterfaceBoardRecvEvent(const QString& linename, const int &infotype, const QByteArray &data);

signals:
    void sendCommandData(const QString &lineName,/*线路名称*/ const int &infotype,/*命令名称*/
                             const QHash<QString,QVariant> &controlDomain,/*控制域*/
                             const QByteArray &data/*数据体*/);
    void sendPrintData(const QString &lineName,/*线路名称*/ const int &infotype,/*命令名称*/
                             const QHash<QString,QVariant> &controlDomain,/*控制域*/
                             const QByteArray &data/*数据体*/);
