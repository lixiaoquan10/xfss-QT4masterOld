    ...
    QWidget(parent), ui(new Ui::CProgramDeviceView)
{
    ...
    ui->tableView->setColumnWidth(DEVICE_COLUMN_TYPE, 240);
    ui->tableView->setColumnWidth(DEVICE_COLUMN_LOGIN_TYPE, 240);
    ui->tableView->setColumnWidth(DEVICE_COLUMN_PRODUCTID, 150);
    ui->tableView->setColumnWidth(DEVICE_COLUMN_CONFIG_LOGIN, 80);
//    ui->tableView->setColumnWidth(DEVICE_COLUMN_AREA, 32);
//    ui->tableView->setColumnWidth(DEVICE_COLUMN_LOCATION, 32);
    ui->tableView->setColumnWidth(DEVICE_COLUMN_STATUS, 76);
...
CDevice *CProgramDeviceView::getDevices(const QModelIndex &index) const
{
    return static_cast<CDevice*>(m_model->getObject(index));
}
        ...
        for(int i=0; i<devices.count(); i++){
            ...
            if(lamp)
            {
                if(((device->deviceTypeId() < 50 && lamp->deviceTypeId() < 50) ||
                        ...
                        && (lamp->getStatus(OBJS_LightCommunicationFault) ||lamp->deviceValue(DEVICE_VALUE_ISUSED) == 0))
                    return true;
            }
        }
        devices.clear();
...
{
    m_index = &index;
    CDevice* device = CGlobal::instance()->programDeviceView()->getDevices(index);
...
//        return;
    ...
    else
    {
        ...
        m_menuTable = createTableMenu(type(device->deviceTypeId()),device);
...
void CProgramDeviceView::slot_Lighten()
{
    ...
    CDevice* device = getDevices(*m_index);
    if(!device)
        return;
    CLoop* loop = device->loop();
    if(!loop)
    ...
    msgDeviceInfo.deviceInfo.insert(DI_KeyId, device->keyId());
...
void CProgramDeviceView::slot_StartEmergency()
{
    ...
    //标志灯开始闪烁
    if(type(device->deviceTypeId()) != NCT_LightType)
    {
        CMsgObjectStatus msgObjectStatus;
        msgObjectStatus.nDisID = distribution->distributionAddress();
        msgObjectStatus.nLoopID = loop->loopAdd();
