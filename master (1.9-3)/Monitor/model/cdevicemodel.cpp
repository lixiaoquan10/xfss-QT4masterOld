...
QVariant CDeviceModel::data(const QModelIndex &index, int role) const
{
    ...
    CLoop* loop = distribution->loopByAdd(device->deviceValue(DEVICE_VALUE_LOOPADDR).toInt());
    if(!device)
        return QVariant();
    if(role== Qt::DisplayRole){
        ...
        else if(index.column() == DEVICE_COLUMN_STATUS)
        {
            ...
            {
                if(device->deviceValue(OBJECT_VALUE_ONLINE) == 1 && loop->loopValue(OBJECT_VALUE_ONLINE) == 1
                        && distribution->distributionValue(OBJECT_VALUE_ONLINE) == 1)
                    return "正常";
                else
                    return "通讯故障";
            }
    ...
    if(role == Qt::BackgroundColorRole){
        ...
        {
            if(device->getStatus(OBJS_LightFault) == 1 || device->deviceValue(OBJECT_VALUE_ONLINE) == 0 ||
                    loop->loopValue(OBJECT_VALUE_ONLINE) == 0 || distribution->distributionValue(OBJECT_VALUE_ONLINE) == 0)
