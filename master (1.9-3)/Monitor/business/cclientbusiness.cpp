...
void CClientBusiness::DeviceBatteryStatus(CDevice* device, int status, int power)
{
    if(device && device->deviceTypeId() < 100){
        ...
        else
        {
            ...
            {
                batteryStatus = "电池断线";
                device->setDeviceStatus(device, OBJS_DeviceBatteryOffFault,1);
                if(device->getStatus(OBJS_DeviceBatteryShortFault))
                    device->setDeviceStatus(device, OBJS_DeviceBatteryShortFault,0);
            }
            else if(status == 0x02)
            {
                batteryStatus = "电池短路";
                device->setDeviceStatus(device, OBJS_DeviceBatteryShortFault,1);
                if(device->getStatus(OBJS_DeviceBatteryOffFault))
                    device->setDeviceStatus(device, OBJS_DeviceBatteryOffFault,0);
            }
            else
            {
                if(device->getStatus(OBJS_DeviceBatteryOffFault))
        ...
        if(power == 0x01)
        {
            ...
            {
                batteryPower = "电池过放电";
                device->setDeviceStatus(device, OBJS_DeviceBatteryOverdischargeFault,1);
            }
            else
            {
                batteryPower = "正常";
        ...
        else
        {
            if(device->getStatus(OBJS_DeviceBatteryOverdischargeFault))
                device->setDeviceStatus(device, OBJS_DeviceBatteryOverdischargeFault,0);
            if(power == 0)
                batteryPower = "正常";
            else if(power == 0x02)
                batteryPower = "%25";
...
//                        distribution->setDistributionValue(DISTRIBUTION_VALUE_DATABASE,0);
                        CLoop* loop;
                        for(int i=1; i<9; i++){
...
//                                    device->setDeviceStatus(device, OBJS_Online, 1);
                                ...
                                device->setDeviceStatus(device, msgObjectStatus->nStatusID, msgObjectStatus->nValue);
...
//                                device->setDeviceValue(DEVICE_VALUE_DATABASE, 0);
                            }
                        }
                        else
                        {
                            if(device->getStatus(OBJS_LightCommunicationFault) == 1)
                                device->setDeviceStatus(device, OBJS_LightCommunicationFault, 0);
                            if(msgObjectStatus->nCommunicationStatus == OBJS_Online)
                            {
//                                device->setDeviceStatus(device, OBJS_Online, 1);
                                device->setDeviceValue(OBJECT_VALUE_ONLINE,1);
...
//                                            distribution->setStatus(OBJS_Online, !msgObjectStatus->nValue);
                                            distribution->setDistributionValue(OBJECT_VALUE_ONLINE,!msgObjectStatus->nValue);
                                                ...
                                                || (msgObjectStatus->nLoopID == loop->loopAdd()))
                                        {
                                            ...
                                            {
                                                loop->setloopValue(OBJECT_VALUE_ONLINE,!msgObjectStatus->nValue);
//                                                loop->setStatus(OBJS_Online, !msgObjectStatus->nValue);
                                    ...
                                    else if(msgObjectStatus->nStatusID == OBJS_LightCommunicationFault)
                                    {
                                        if(msgObjectStatus->nLoopID == loop->loopAdd())
                                        {
                                            ...
                                            //配置过的灯具才会报故障
                                            if(device && (device->deviceValue(OBJECT_VALUE_ONLINE) == msgObjectStatus->nValue) &&
                                                    (device->deviceValue(DEVICE_VALUE_ISUSED) == 1 || device->deviceValue(DEVICE_VALUE_ISUSED) == 0))
                                            {
                                                if(!device->isDeviceForbid())
                                                {
//                                                    device->setDeviceStatus(device, OBJS_Online, !msgObjectStatus->nValue);
                                                    device->setDeviceValue(OBJECT_VALUE_ONLINE,!msgObjectStatus->nValue);
                                                    device->setDeviceStatus(device, OBJS_LightCommunicationFault, msgObjectStatus->nValue);
                                    ...
                                    else if(msgObjectStatus->nStatusID == OBJS_LightFault)
                                    {
                                        if(msgObjectStatus->nLoopID == loop->loopAdd())
                                        {
                                                    ...
                                                    (device->deviceValue(DEVICE_VALUE_ISUSED) == 1 || device->deviceValue(DEVICE_VALUE_ISUSED) == 0))
                                            {
                                                if(!device->isDeviceForbid())
                                                {
                                                    device->setDeviceStatus(device, OBJS_LightFault, msgObjectStatus->nValue);
                                                    if(msgObjectStatus->nValue == 1)
...
//                device->setDeviceStatus(device, OBJS_Online, 1);
//                device->setDeviceStatus(device, OBJS_LightCommunicationFault, 0);
//                device->setDeviceStatus(device, OBJS_LightFault, 0);
                device->setDeviceValue(DEVICE_VALUE_EMERGENCY,"正常");
                device->setDeviceValue(DEVICE_VALUE_LIGHT,"正常");
                ...
                for(int i=0; i<loops.count(); i++){
                    ...
                    if(loop){
                        for(int m=0; m<64; m++){
                            ...
                            if(device && (device->deviceValue(DEVICE_VALUE_TYPE_ID) != 0))
                            {
...
{
    QString data;
    data = "CClientBusiness::objectStatusChanged" +  QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz") + "\n";
    ...
    if(objectType == CController::type_s)
    {
        ...
        m_statusInfo.area = controller->controllerValue(DEVICE_VALUE_AREA).toString();
        m_statusInfo.location = controller->controllerValue(DEVICE_VALUE_LOCATION).toString();
    }
    else if(objectType == CDistribution::type_s)
    {
        ...
        m_statusInfo.canID = distribution->distributionValue(DISTRIBUTION_VALUE_ADDR).toInt();
        m_statusInfo.loopID = 0;
        m_statusInfo.deviceID = 0;
        m_statusInfo.area = distribution->distributionValue(DISTRIBUTION_VALUE_AREA).toString();
        m_statusInfo.location = distribution->distributionValue(DISTRIBUTION_VALUE_LOCATION).toString();
    }
    else if(objectType == CLoop::type_s)
    {
        ...
        m_msgLog->Remark = CGlobal::instance()->getObjectStatusStr(status) + CGlobal::instance()->getObjectStatusSuffix(status, newValue);
        m_msgLog->Canport = 1;
        m_msgLog->CanDeviceAddress = loop->loopValue(LOOP_VALUE_DISTRIBUTIONADDR).toInt();
        m_msgLog->Loop = loop->loopValue(LOOP_VALUE_ADDR).toInt();
        m_msgLog->LampDeviceAddress = 0;
        m_msgLog->title = strtext;
        if(type != SRT_Emergency)
            exeCommand(NCT_AddLog, m_msgLog);
        m_statusInfo.canID = loop->loopValue(LOOP_VALUE_DISTRIBUTIONADDR).toInt();
        m_statusInfo.loopID = loop->loopValue(LOOP_VALUE_ADDR).toInt();
        m_statusInfo.deviceID = 0;
        m_statusInfo.area = "";
    ...
    data = data + "end  CClientBusiness::objectStatusChanged" +  QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz") + "\n";
...
//                        loop->setStatus(OBJS_Online,0);
                        ...
                        for(int m=0; m<devices.count(); m++){
                            ...
                            if(device)
                            {
//                                device->setDeviceStatus(device, OBJS_Online,0);
...
//            controller->setStatus(OBJS_Online,0);
            controller->setControllerValue(OBJECT_VALUE_ONLINE,0);
        }
        CGlobal::instance()->InformationWindow()->hide();
    }
}
...
CController *CClientBusiness::createUndefinedController(CProject *project, int address, int keyId)
{
    ...
    controller->setControllerValue(CONTROLLER_VALUE_NAME, controller->controllerValue(CONTROLLER_VALUE_NAME).toString().append(QString::number(address)));
