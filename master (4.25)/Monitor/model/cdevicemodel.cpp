...
QVariant CDeviceModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role== Qt::DisplayRole){
        ...
        else if(index.column() == DEVICE_COLUMN_LOGIN_TYPE)
        {
            ...
            //配且注
            else if(device->deviceValue(DEVICE_VALUE_ISUSED) == 1 && loop->isLoopLogin()
                    && device->isDeviceLogin() && distribution->isDistributionLogin())
            {
            ...
            //仅配置//未定义
                     ...
                     || !loop->isLoopLogin() || !distribution->isDistributionLogin()){
                devType = CGlobal::instance()->deviceType(0);
            }
            if(!devType)
                devType = CGlobal::instance()->deviceType(0);
        ...
        else if(index.column() == DEVICE_COLUMN_CONFIG_LOGIN)
        {
            if(device->deviceValue(DEVICE_VALUE_ISUSED) == 2)
                ...
                return "未定义";
            else if(device->deviceValue(DEVICE_VALUE_ISUSED) == 0 || !loop->isLoopLogin()
                    || !distribution->isDistributionLogin() || !device->isDeviceLogin())
                return "仅配置";
        }
        //灯具状态
