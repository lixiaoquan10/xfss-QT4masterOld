...
void dlgDevices::getDeviceInfo()
{
    int count = 0;
    CController* controller = CGlobal::instance()->project()->controllerById(CGlobal::instance()->panelAddress());
    ui->m_deviceNumber->setText(QString::number(controller->getDeviceTotal()));
    ui->pageScrollBar->setMaximum(controller->getDeviceTotal()/13);
    //灯具计数
    int deviceCount = 0;
    //当前需要显示的灯具计数值
    int currentDeviceNumber = ui->pageScrollBar->value()*13;
    for(int i=1; i<63; i++)
    {
            ...
            for(int i=1; i<9; i++){
                ...
                if(loop)
                {
                    for(int m=1; m<256; m++){
                        ...
                        {
                            deviceCount++;
                            if((deviceCount > currentDeviceNumber) && (deviceCount <= currentDeviceNumber+13))
                            {
                                ...
                                if(deviceCount == currentDeviceNumber+13)
                                    return;
                            }
                        }
                    }
