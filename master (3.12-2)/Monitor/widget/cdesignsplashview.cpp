...
//    QString strlamptofirepoint = QString("SELECT l.FirePointID,l.LayoutPageID,l.CX,l.CY,l.Width,l.Height,r.ID,r.Device,r.Loop,r.Address from FirePointCoordinate l "
//                              "left join FirePoint r on l.FirePointID = r.ID "
//                              "where l.LayoutPageID=%1 "
//                              ).arg(layerID);
...
//    double width = firepoint->firePointValue(FIREPOINT_VALUE_WIDTH).toDouble()*m_changeValue/100;
    ...
    if(CGlobal::instance()->ClientBusiness()->isFirepointEmergency(firepoint) || CGlobal::instance()->m_isEmergency)
    {
        //设置填充颜色为Red
...
//    QPointF firepointOffset(0,0);
    ...
    painter.setBrush(QBrush(QColor(255,255,0)));  // 设置填充颜色为Brown3
    // 创建一个 QPen 对象，并设置其样式为虚线
    QPen pen;
    pen.setColor(Qt::black); // 设置边框颜色为黑色
    pen.setStyle(Qt::DashLine); // 设置样式为虚线

    // 将该 QPen 对象应用到 QPainter 中
    painter.setPen(pen);
