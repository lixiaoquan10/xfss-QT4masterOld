    ...
    ui(new Ui::dlgTimeInterval)
{
    ...
    startRuleGroup->addButton(ui->onlyStartDistributionCheckBox);
...
//    ui->label_monthtime->setText(QString::number(CGlobal::instance()->m_MonthCheckSpanSecond));
    ...
    ui->SoundBox->hide();
    ui->MYcheckBox->hide();
    ui->MYcheckTimeBox->hide();

    QIcon icon(":/images/startinfo.png"); // 图标路径
    TriangleButton *triangleButton = new TriangleButton(icon,this);
    // 设置按钮样式表，隐藏按钮的边框和背景
    triangleButton->setStyleSheet("border: none; background: none;");
    triangleButton->setGeometry(1320,450,32,32);
    connect(triangleButton, SIGNAL(clicked()), this, SLOT(on_triangleButton_clicked()));
...
//    CGlobal::instance()->m_MonthCheckSpanSecond = ui->MonthTimeSlider->value();
    ...
    CGlobal::instance()->m_version_can5can6 = ui->version_can5can6->text();
    CGlobal::instance()->m_version_can7can8 = ui->version_can7can8->text();
...
void dlgTimeInterval::on_onlyStartDistributionCheckBox_clicked()
{
    CGlobal::instance()->m_nStartRule = RULE_OnlyDistribution;
}

void dlgTimeInterval::on_Can4CheckBox_clicked()
{
//    CGlobal::instance()->m_nCanNumber = 4;
    QString fileName = CGlobal::instance()->workspaces() + "/" + "ESSQLiteCE100.db";
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");;
    db.setDatabaseName(fileName);
    if(!db.open())
...
{
//    CGlobal::instance()->m_nCanNumber = 8;
    QString fileName = CGlobal::instance()->workspaces() + "/" + "ESSQLiteCE100.db";
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");;
    db.setDatabaseName(fileName);
    if(!db.open())
...
void ipsetThread::run()
{
    ...
    arguments << CGlobal::instance()->TimeInterval()->m_getTmpip1 << CGlobal::instance()->TimeInterval()->m_getTmpmask1
              << CGlobal::instance()->TimeInterval()->m_getTmpgateway1 ;
//              << CGlobal::instance()->TimeInterval()->m_getTmpip2
...
void dlgTimeInterval::on_pushButton_saveWlan1_clicked()
{
    ...
    if (file.open(QIODevice::Append | QIODevice::Text))
    {
        ...
        if(isWLAN1Changed())
        {
            QString str = "////" + QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss:zzz") + "\n" +
                    ...
                    "子网掩码:" + ui->m_subnetMask1->text() + "\n" +
                    "网关:" + ui->m_gateway1->text() + "\n" + "\n";
            out << str;  // 写入数据
        }
    ...
    m_getTmpmask1 = ui->m_subnetMask1->text();
    m_getTmpgateway1 = ui->m_gateway1->text();
    setWLANData();
}

void dlgTimeInterval::on_pushButton_saveWlan2_clicked()
{
    QFile file("home/xfss/root/IP修改记录.txt");
    ...
    if (file.open(QIODevice::Append | QIODevice::Text))
    {
        ...
        {
            QString str = "////" + QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss:zzz") + "\n" +
                    "网口2地址:" + ui->m_myip2->text() + "\n" +
                    "子网掩码:" + ui->m_subnetMask2->text() + "\n" +
                    "网关:" + ui->m_gateway2->text() + "\n" + "\n";
            out << str;  // 写入数据
        }
    ...
    m_getTmpmask2 = ui->m_subnetMask2->text();
...
bool dlgTimeInterval::isWLAN1Changed()
{
    if(ui->m_myip1->text() != m_getTmpip1)
        return true;
    if(ui->m_subnetMask1->text() != m_getTmpmask1)
        return true;
...
bool dlgTimeInterval::isWLAN2Changed()
{
    if(ui->m_myip2->text() != m_getTmpip2)
        return true;
    if(ui->m_subnetMask2->text() != m_getTmpmask2)
        return true;
...
//        ui->VersionBox->setGeometry(1000,130,270,140);
//        ui->pingBox->setGeometry(1000,310,270,100);
    }
...
void dlgTimeInterval::updateWLANData()
{
            ...
            foreach (QNetworkAddressEntry entry, entries) {
                if (entry.ip().protocol() == QAbstractSocket::IPv4Protocol)
                {
                    ...
                    if(interfaceName == "enp1s0")
                    {
                        ...
                        m_getTmpmask1 = entry.netmask().toString();

                        ui->m_myip1->setText(m_getTmpip1);
                        ui->m_subnetMask1->setText(m_getTmpmask1);
                    }
                    else if(interfaceName == "enp4s0")
                    {
                        m_getTmpip2 = entry.ip().toString();
                        m_getTmpmask2 = entry.netmask().toString();

                        ui->m_myip2->setText(m_getTmpip2);
                        ui->m_subnetMask2->setText(m_getTmpmask2);
                    }
                }
            }
        }
    ...
    // 获取 enp1s0 接口的网关
    QString gateway1 = getGateway("enp1s0");
    qDebug() << "Gateway for enp1s0:" << gateway1;
    m_getTmpgateway1 = gateway1;
    ui->m_gateway1->setText(m_getTmpgateway1);

    // 获取 enp4s0 接口的网关
    QString gateway2 = getGateway("enp4s0");
    qDebug() << "Gateway for enp4s0:" << gateway2;
    m_getTmpgateway2 = gateway2;
