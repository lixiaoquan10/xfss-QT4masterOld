...
//                        distribution->setDistributionValue(DISTRIBUTION_VALUE_NOLOGIN,1);
                                    ...
                                    else
                                    {
                                        ...
                                        for(int m=0; m<devices.count(); m++){
                                            ...
                                            if(device->isDeviceOnline())
                                            {
                                                ...
                                                if(!checkDevice || device->deviceValue(DEVICE_VALUE_ISUSED) == 0)
                                                {
...
void CDataProcessing::selfCheckStop(QList<QVariant> data)
{
    ...
    if(CGlobal::instance()->m_LiftTime <= 15)
    {
        QTimer::singleShot(60000, this, SLOT(slot_usetime1h()));
    }
}
    ...
    for(int i=0; i<m_informationModel.count(); i++)
    {
        ...
        if(count > 0)
        {
            for(int i=0; i < count; i++)
            {
                ...
                if(canData == "")
                {
                    m_informationModel.value(name)->removeData(m_informationModel.value(name)->getStatusInfo(0).keyId);
                    continue;
                }
                ...
                {
                    if(m_informationModel.value(name)->getStatusInfo(0).statusId == 0)
                    {
                        m_informationModel.value(name)->removeData(m_informationModel.value(name)->getStatusInfo(0).keyId);
                        continue;
                    }
                ...
                else
                {
                    ...
                    {
                        if(m_informationModel.value(name)->getStatusInfo(0).distributionId != "")
                        {
                            CDistribution* distribution = canport->distributionByAddress(distributionAddress);
                            if(!distribution)
                                continue;
