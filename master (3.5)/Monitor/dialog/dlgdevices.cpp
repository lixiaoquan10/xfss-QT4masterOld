...
void dlgDevices::getDeviceInfo()
{
    ...
    for(int i=3; i<=6; i++)
    {
        ...
        int number = 0;
        //状态显示页面存在时每页显示13条，不存在时显示20条
        if(CGlobal::instance()->DataProcessing()->isStatusTab())
            number = 13;
        else
            number = 20;
        int maxPage = deviceTotal%number > 0 ? (deviceTotal/number+1) : (deviceTotal/number);
        ui->pageScrollBar->setRange(1, maxPage);
        //灯具计数
        int deviceCount = 0;
        //当前需要显示的灯具计数范围
        int currentDeviceMinNumber = ui->pageScrollBar->value()*number - number;
        int currentDeviceMaxNumber = ui->pageScrollBar->value()*number;
