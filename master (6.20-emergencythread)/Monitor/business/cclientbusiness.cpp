            ...
            {   //状态判断数据
                ...
                if(msgObjectStatus->nStatusID == OBJS_FireEmergency)
                {
                    ...
                    if(firepoint)
                    {
                        ...
                        else if(CGlobal::instance()->m_nStartRule == RULE_OnlyDistribution)
...
//            serverManualLaunchUpload();
        ...
        case NCT_MonthCheckBegin:
        {
            ...
            {
                QString str = QString("%1秒").arg(time, 2, 10, QChar('0')); // 格式化秒数，宽度2，用'0'填充
...
void CClientBusiness::slot_threadFinished()
{
    // 获取发送信号的线程对象
...
//        uploadLampStatus(device);
        ...
        {
            if(((!CGlobal::instance()->ClientBusiness()->isUCLamp(lamptypeID)) && (CGlobal::instance()->m_SendDirectionAndTwinkleByOne == true))
                    || (CGlobal::instance()->ClientBusiness()->isUCLamp(lamptypeID) && (CGlobal::instance()->m_UCSendDirectionAndTwinkleByOne == true)))
            {
                direction |= 0x08;
            }
            else
        ...
        if(lamptypeID == 109 || lamptypeID == 11 || lamptypeID == 146)   //109/11/146代表可变状态灯
        {
            ...
            CGlobal::instance()->ClientBusiness()->DeviceStartStatus(msgObjectStatus.nCanportAdd, msgObjectStatus.nDisID, msgObjectStatus.nLoopID, msgObjectStatus.nDeviceID,
...
{
    m_firePointID = firePointID;
    firePointEmergencyByGroupThread *thread = new firePointEmergencyByGroupThread(this);
    connect(thread, SIGNAL(threadFinished()), this, SLOT(slot_threadFinished()), Qt::QueuedConnection);
    thread->start();
}

void firePointEmergencyByGroupThread::run()
{
    int direction = 0;
    ...
    QSqlQuery queryGroup = db.query(QString("SELECT g.LaunchGroupID from LampToLaunchGroup g "
                                       ...
                                       " where f.FirePointID=%1").arg(CGlobal::instance()->ClientBusiness()->m_firePointID));
...
//            uploadLampStatus(device);
        ...
        QSqlQuery querylampcontrol = db.query(QString("SELECT l.LampDirection,r.Remark,r.Address,c.Loop,a.CanPort,a.Address,r.id,r.LampTypeID,r.IsReverse,l.FirePointID from EvacuationPathItem l "
                                           ...
                                           " where g.LaunchGroupID=%1 and l.FirePointID=%2").arg(*it).arg(CGlobal::instance()->ClientBusiness()->m_firePointID));
        //query.value(0)代表第0列
        //第0列:l.LampDirection  灯具方向
        //第1列:r.Remark         灯具产品编号
        //第2列:r.Address        灯具地址
...
{
    m_firePointID = firePointID;
    firePointEmergencyByLayoutPageThread *thread = new firePointEmergencyByLayoutPageThread(this);
    connect(thread, SIGNAL(threadFinished()), this, SLOT(slot_threadFinished()), Qt::QueuedConnection);
    thread->start();
}

void firePointEmergencyByLayoutPageThread::run()
{
    int direction = 0;
    QString fileName = CGlobal::instance()->workspaces() + "/" + "ESSQLiteCE100.db";
    CDBSqlite db(fileName);
    ...
    for (QSet<int>::const_iterator it = layoutPages.begin(); it != layoutPages.end(); ++it) {
        ...
        QSqlQuery querylampcontrol = db.query(QString("SELECT l.LampDirection,r.Remark,r.Address,c.Loop,a.CanPort,a.Address,r.id,r.LampTypeID,r.IsReverse,l.FirePointID from EvacuationPathItem l "
                                           ...
                                           " where p.LayoutPageID=%1 and l.FirePointID=%2").arg(*it).arg(CGlobal::instance()->ClientBusiness()->m_firePointID));
        //query.value(0)代表第0列
        //第0列:l.LampDirection  灯具方向
        //第1列:r.Remark         灯具产品编号
        //第2列:r.Address        灯具地址
...
{
    m_firePointID = firePointID;
    firePointEmergencyByDistributionThread *thread = new firePointEmergencyByDistributionThread(this);
    connect(thread, SIGNAL(threadFinished()), this, SLOT(slot_threadFinished()), Qt::QueuedConnection);
    thread->start();
}

void firePointEmergencyByDistributionThread::run()
{
    int direction = 0;
    QString fileName = CGlobal::instance()->workspaces() + "/" + "ESSQLiteCE100.db";
    CDBSqlite db(fileName);
...
//                device->setDeviceControl(true);
        ...
        QSqlQuery querylampcontrol = db.query(QString("SELECT l.LampDirection,r.Remark,r.Address,c.Loop,a.CanPort,a.Address,r.id,r.LampTypeID,r.IsReverse,l.FirePointID from EvacuationPathItem l "
                                           ...
                                           " left join ChannelInfo c on r.ChannelInfoID = c.ID "
                                           " left join CanDeviceInfo a on c.CanDeviceInfoID = a.ID "
                                           " where a.CanPort=%1 and a.Address=%2 and l.FirePointID=%3").arg(pair.first).arg(pair.second).arg(CGlobal::instance()->ClientBusiness()->m_firePointID));
        //query.value(0)代表第0列
        //第0列:l.LampDirection  灯具方向
        //第1列:r.Remark         灯具产品编号
        //第2列:r.Address        灯具地址
...
//                statusInfo.description = device->descriptionStr();
                ...
                CGlobal::instance()->DataProcessing()->slotStateInformation(statusInfo, SRT_Start, SRO_Add);
            }
        }
    }
}
