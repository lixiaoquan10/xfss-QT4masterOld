...
int CStyleHelper::clamp(float x)
{
    const int val = x > 255 ? 255 : static_cast<int>(x);
    return val < 0 ? 0 : val;
}

QColor CStyleHelper::mergedColors(const QColor &colorA, const QColor &colorB, int factor)
{
...
qreal CStyleHelper::sidebarFontSize()
{
#if defined(Q_OS_MAC)
    return 10;
#else
...
QPalette CStyleHelper::sidebarFontPalette(const QPalette &original)
{
...
QColor CStyleHelper::panelTextColor(bool lightColored)
{
...
QColor CStyleHelper::m_baseColor;
QColor CStyleHelper::m_requestedBaseColor;
QSet<const QWidget*> CStyleHelper::m_allowWidgets;

QColor CStyleHelper::baseColor(bool lightColored)
{
    if (!lightColored)
        return m_baseColor;
    else
...
QColor CStyleHelper::highlightColor(bool lightColored)
{
...
QColor CStyleHelper::shadowColor(bool lightColored)
{
    QColor result = baseColor(lightColored);
    result.setHsv(result.hue(),
...
QColor CStyleHelper::borderColor(bool lightColored)
{
    QColor result = baseColor(lightColored);
    result.setHsv(result.hue(),
                  result.saturation(),
...
void CStyleHelper::setBaseColor(const QColor &newcolor)
{
...
void CStyleHelper::verticalGradientHelper(QPainter *p, const QRect &spanRect, const QRect &rect, bool lightColored)
{
    QColor highlight = CStyleHelper::highlightColor(lightColored);
    QColor shadow = CStyleHelper::shadowColor(lightColored);
...
void CStyleHelper::verticalGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect, bool lightColored)
{
    if (CStyleHelper::usePixmapCache()) {
        QString key;
...
void CStyleHelper::horizontalGradientHelper(QPainter *p, const QRect &spanRect, const
QRect &rect, bool lightColored)
{
    ...
    QColor base = CStyleHelper::baseColor(lightColored);
    QColor highlight = CStyleHelper::highlightColor(lightColored);
    QColor shadow = CStyleHelper::shadowColor(lightColored);
    QLinearGradient grad(rect.topLeft(), rect.bottomLeft());
    grad.setColorAt(0, highlight.lighter(120));
    if (rect.height() == CStyleHelper::navigationWidgetHeight()) {
...
void CStyleHelper::horizontalGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect, bool lightColored)
{
    if (CStyleHelper::usePixmapCache()) {
        QString key;
        QColor keyColor = baseColor(lightColored);
...
void CStyleHelper::menuGradientHelper(QPainter *p, const QRect &spanRect, const QRect &rect)
{
    QLinearGradient grad(spanRect.topLeft(), spanRect.bottomLeft());
    QColor menuColor = CStyleHelper::mergedColors(CStyleHelper::baseColor(), QColor(244, 244, 244), 25);
    grad.setColorAt(0, menuColor.lighter(112));
    grad.setColorAt(1, menuColor);
    p->fillRect(rect, grad);
}

void CStyleHelper::drawArrow(QStyle::PrimitiveElement element, QPainter *painter, const QStyleOption *option)
{
    ...
    pixmapName.append(QString::number(option->palette.cacheKey()));
...
void CStyleHelper::menuGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect)
{
    if (CStyleHelper::usePixmapCache()) {
        QString key;
        key.sprintf("mh_menu %d %d %d %d %d",
            spanRect.width(), spanRect.height(), clipRect.width(),
            clipRect.height(), CStyleHelper::baseColor().rgb());
...
void CStyleHelper::drawIconWithShadow(const QIcon &icon, const QRect &rect,
...
void CStyleHelper::drawCornerImage(const QImage &img, QPainter *painter, QRect rect,
...
void CStyleHelper::tintImage(QImage &img, const QColor &tintColor)
{
    QPainter p(&img);
