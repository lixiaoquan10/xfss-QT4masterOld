...
//        emit sendLedData(communication, SetLedStatus, data);
        m_communicationManager->sendLedData(communication, SetLedStatus, data);
    }
}
...
{
    if(type == CT_LedCard || type == CT_CanCard1 || type == CT_CanCard2 || type == CT_LinkageCard || type == CT_FASCard)
        return true;
    else
        return false;
}
...
void CCommunicationRoute::procProcessSendEvent(const QString &processname, const int &infoType, const QHash<QString, QVariant> &control, const QByteArray &data)
{
    if(m_processCommunication.contains(processname))
    {
        ...
        foreach(QString linename, list){
            ...
            {
                //cancard1:canport3/canport4   cancard2:canport5/canport6             canportAdd == 0广播
                if(!canportAdd || (linename == "cancard1" && canportAdd < 5) || (linename == "cancard2" && canportAdd >= 5))
