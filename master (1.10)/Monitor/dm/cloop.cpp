...
bool CLoop::setloopValue(QString name, QVariant value)
{
    ...
    {
        if(value == 1 && loopValue(OBJECT_VALUE_ONLINE) == 0)
            setDeviceOnlineTotal(m_devices.count());
        else if(value == 0 && loopValue(OBJECT_VALUE_ONLINE) == 1)
...
bool CLoop::getLoopCommunicationFault()
{
    ...
    CDistribution* distribution = controller->distributionByAddress(loopValue(LOOP_VALUE_DISTRIBUTIONADDR).toInt());
