...
//    data = data + "CGlobal::instance()->designSplashView()->setPageStateUpdate(true);" +  QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz") + "\n";
    ...
    for(int i=0; i<m_informationModel.count(); i++)
    {
        ...
        if(count > 0)
        {
            for(int i=0; i < count; i++)
            {
                ...
                {
                    CDistribution* distribution = controller->distributionByAddress(m_informationModel.value(name)->getStatusInfo(i).canID);
                ...
                else
                {
                    ...
                    {
                        CDistribution* distribution = controller->distributionByAddress(m_informationModel.value(name)->getStatusInfo(0).canID);
                        CLoop* loop = distribution->loopByAdd(m_informationModel.value(name)->getStatusInfo(0).loopID);
