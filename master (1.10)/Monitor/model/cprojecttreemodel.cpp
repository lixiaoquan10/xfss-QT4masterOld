...
QVariant CProjectTreeModel::getDecorationData(const QModelIndex &index) const
{
    ...
    if(objectType == CController::type_s)
    {
        ...
        return QPixmap(controller->controllerValue(OBJECT_VALUE_PIXMAP).toString()).scaled(16,16);
    }
...
QVariant CProjectTreeModel::getTextColorData(const QModelIndex &index) const
{
    ...
    {
        CLoop* loop = static_cast<CLoop*>(data);
        return ((loop->loopValue(OBJECT_VALUE_ONLINE)==1) && (data->value(OBJECT_VALUE_ONLINE_COUNT).toInt() > 0)) ? QColor(0,139,0) : QColor(139,125,107);
    }
    if(data->type() == CDistribution::type_s)
    {
        CDistribution* distribution = static_cast<CDistribution*>(data);
        return ((data->getStatus(OBJS_DistributionCommunicationFault) == 1) || (distribution->distributionValue(OBJECT_VALUE_ONLINE) == 0)) ? QColor(139,125,107) : QColor(0,139,0);
    }
    if(data->type() == CController::type_s)
    {
        CController* controller = static_cast<CController*>(data);
        return ((controller->controllerValue(OBJECT_VALUE_ONLINE)==1) || (data->value(OBJECT_VALUE_ONLINE_COUNT).toInt() > 0)) ? QColor(0,139,0) : QColor(139,125,107);
    }
