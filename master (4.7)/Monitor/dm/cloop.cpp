...
void CLoop::setDeviceOnlineTotal(int n, bool isloopfault)
{
    if(m_deviceOnlineTotal == n)
        return;
    int adj = n - m_deviceOnlineTotal;
    adjustedDeviceOnlineTotal(adj, isloopfault);
    //灯具数量备份
    if(n != 0)
    {
...
void CLoop::adjustedDeviceOnlineTotal(int adj, bool isloopfault)
{
