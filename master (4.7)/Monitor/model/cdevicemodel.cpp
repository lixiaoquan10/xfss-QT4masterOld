...
QVariant CDeviceModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role== Qt::DisplayRole){
        ...
        {
            //仅注册或者配且注
            if((device->deviceValue(DEVICE_VALUE_ISUSED) == 1 && loop->loopValue(LOOP_VALUE_NOLOGIN) != 1
                && device->deviceValue(DEVICE_VALUE_NOLOGIN) != 1 && distribution->distributionValue(DISTRIBUTION_VALUE_NOLOGIN) != 1)
        ...
        {
            //仅注册//未定义
            if(device->deviceValue(DEVICE_VALUE_ISUSED) == 2 || device->deviceValue(DEVICE_VALUE_ISUSED) == 3)
        ...
        else if(index.column() == DEVICE_COLUMN_LOGIN_TYPE)
        {
            ...
            //配且注
            else if(device->deviceValue(DEVICE_VALUE_ISUSED) == 1 && loop->loopValue(LOOP_VALUE_NOLOGIN) != 1
                    && device->deviceValue(DEVICE_VALUE_NOLOGIN) != 1
        ...
        else if(index.column() == DEVICE_COLUMN_CONFIG_LOGIN)
        {
            if(device->deviceValue(DEVICE_VALUE_ISUSED) == 2)
                ...
                return "未定义";
            else if(device->deviceValue(DEVICE_VALUE_ISUSED) == 0 || loop->loopValue(LOOP_VALUE_NOLOGIN) == 1
                    || distribution->distributionValue(DISTRIBUTION_VALUE_NOLOGIN) == 1)
                return "仅配置";
        }
        //灯具状态
        ...
        {
            //清除注册后未注册到的灯具
            if(loop->isRemoveLogin() && device->deviceValue(DEVICE_VALUE_ISUSED) == 0)
            {
                 return "通讯故障";
            }
            //非故障的灯具
            else if(device->getDeviceCommunicationFault() == false)
            {
    ...
    if(role == Qt::BackgroundColorRole){
        ...
        if(index.column() == DEVICE_COLUMN_STATUS)
        {
            ...
            else if(device->getStatus(OBJS_LightFault) == 1 || device->getDeviceCommunicationFault())
                return QColor(245,222,179); //黄色
