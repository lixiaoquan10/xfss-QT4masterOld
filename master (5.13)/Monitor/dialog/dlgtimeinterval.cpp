    ...
    ui(new Ui::dlgTimeInterval)
{
    ...
    triangleButton->setGeometry(1280,450,32,32);
...
{
    //跳转到usetime界面
    CGlobal::instance()->centerFrameContainer()->setCurrentIndex(6);
    // 设置窗口为模态对话框
    setModal(true);
    //显示 TimeInterval 窗口
    CGlobal::instance()->TimeInterval()->show();
    // 将 TimeInterval 界面设为不可用
    CGlobal::instance()->TimeInterval()->setEnabled(false);
}
    ...
    if(process.waitForFinished())
    {
        ...
        while (!in.atEnd())
        {
            ...
            if (line.contains("iface enp1s0 inet static"))
            {
                ...
                lines.append("address " + CGlobal::instance()->TimeInterval()->m_getTmpip);
                lines.append("netmask " + CGlobal::instance()->TimeInterval()->m_getTmpmask);
                lines.append("gateway " + CGlobal::instance()->TimeInterval()->m_getTmpgateway);
                lines.append("dns-nameservers 8.8.8.8");
            }
            else if(line.contains("iface enp1s1 inet static"))
            {
                shouldAddExtraContent = false;
...
void dlgTimeInterval::updateSqliteData()
{
    ...
    if(CGlobal::instance()->m_nCanNumber == 4)
    {
        ...
        ui->VersionBox->setGeometry(50,130,275,140);
        ui->pingBox->setGeometry(50,310,270,100);
    }
