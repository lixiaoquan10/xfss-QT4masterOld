...
CCanport *CClientBusiness::currentPanel() const
{
    ...
    if(CGlobal::instance()->dm()->isLoad())
        canport = CGlobal::instance()->dm()->projectAt(0)->canportById(CGlobal::instance()->panelAddress());
    return canport;
}
...
void CClientBusiness::recvData(int nMsgType, QByteArray &data)
{
    ...
    switch(type)
    {
        case NCT_ObjectStatus:
        {
            ...
            CCanport* canport = CGlobal::instance()->project()->canportById(msgObjectStatus->nPanelID);
            if(!canport)
                canport = createUndefinedCanport(CGlobal::instance()->project(), msgObjectStatus->nPanelID);
            if(msgObjectStatus->nType == 0x01){
                canport->setStatus(msgObjectStatus->nStatusID, msgObjectStatus->nValue);
...
void CClientBusiness::Emergency()
{
    ...
    while(query.next())
    {
        ...
        if(!canport)
            continue;
        CDistribution* distribution = canport->distributionByAddress(query.value(3).toInt());
...
//        device->setDeviceValue(DEVICE_VALUE_EMERGENCY,"应急");
    ...
    while(query1.next())
    {
        ...
        if(!distribution || distribution->getStatus(OBJS_DistributionCommunicationFault))
            continue;
        CLoop* loop = distribution->loopByAdd(query.value(3).toInt());
        if(!loop || loop->getStatus(OBJS_LoopCommunicationFault) || loop->loopValue(LOOP_VALUE_NOLOGIN) == 1)
            continue;
        CDevice* device = loop->deviceByAdd(query.value(2).toInt());
        if(!device || device->getStatus(OBJS_LightCommunicationFault))
            continue;
...
CCanport *CClientBusiness::createUndefinedCanport(CProject *project, int address, int keyId)
{
