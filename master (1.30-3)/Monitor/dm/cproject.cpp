...
CProject::~CProject()
{
    ...
    for(int i = 0; i < m_canports.size(); ++i)
        m_canports.at(i)->setProject(NULL);
...
int CProject::loadProject(CDBSqlite *db, CDM *dm)
{
    ...
    project->setValue(PROJECT_VALUE_NAME, "控制器");
    project->setStatus(OBJS_Online,1);
    project->setNone();
    dm->addProject(project);
    if(CCanport::loadCanport(db, project) != 0)
        return -1;
...
bool CProject::addCanport(CCanport* canport)
{
    if(!canport)
        return false;
...
void CProject::removeCanport(CCanport* canport)
{
    if(!canport)
        return;
    if(!m_canports.contains(canport))
...
CCanport *CProject::canportById(const int id) const
{
    for(int i = 0; i < m_canports.size(); ++i)
    {
...
bool CProject::hasCanport(const int id) const
{
...
void CProject::initProject()
{
    ...
    setValue(PROJECT_VALUE_NAME, "控制器");
...
int CProject::saveCanport(CDBSqlite *db)
{
    for(int i = 0; i < m_canports.size(); ++i)
    {
...
void CProject::addDeviceTypeAndOnline(CCanport *canport)
{
...
void CProject::removeDeviceTypeAndOnline(CCanport *canport)
{
    QHash<int, int> tmp = canport->getDeviceTypeNumber();
    QHash<int, int>::const_iterator iter = tmp.constBegin();
    while(iter != tmp.constEnd())
