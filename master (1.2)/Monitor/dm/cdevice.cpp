...
//              ");").exec();
//    db->query("CREATE UNIQUE INDEX IF NOT EXISTS idx_device ON device ("
//                  "parent_keyid ASC,"
//                  "device_id ASC"
//              ");").exec();
...
int CDevice::loadDevice(CDBSqlite *db, CLoop *loop)
{
    ...
    while(query.next())
    {
        ...
        device->setValue(DEVICE_VALUE_ID, query.value(fieldKeyId).toInt());
        device->setValue(DEVICE_VALUE_TYPE_ID, query.value(fieldTypeId).toInt());
        device->setValue(DEVICE_VALUE_DESCRIPTION, query.value(fieldRemark));
        device->setValue(DEVICE_VALUE_ADDR, query.value(fieldAddress).toInt());
        device->setValue(DEVICE_VALUE_TYPE, query.value(fieldDeviceType));
        device->setValue(DEVICE_VALUE_ISREVERSE, query.value(fieldReverse).toInt());
        device->setValue(DEVICE_VALUE_ISUSED, query.value(fieldUsed).toInt());
        device->setValue(DEVICE_VALUE_AREA, query.value(fieldArea));
        device->setValue(DEVICE_VALUE_LOCATION, query.value(fieldLocation));

        device->setValue(DEVICE_VALUE_LOOPADDR, loop->loopAdd());
        device->setValue(DEVICE_VALUE_DISTRIBUTIONADDR, loop->distribution()->distributionAddress());

        device->setValue(DEVICE_VALUE_NAME, QString("%1_%2").arg(device->value(DEVICE_VALUE_TYPE).toString()).arg(device->value(DEVICE_VALUE_ADDR).toInt()));
        device->setValue(DEVICE_VALUE_EMERGENCY, "正常");
        device->setValue(DEVICE_VALUE_LIGHT, "正常");
        device->setValue(DEVICE_VALUE_DEFAULTDIRECTION,query.value(fieldLampDirection).toInt());
...
bool CDevice::setSecondCode(QString &secondCode)
{
    ...
    if(!setValue(DEVICE_VALUE_SECONDCODE, secondCode.rightJustified(8, '0', true)))
        return false;
...
QVariant CDevice::afterGetValue(QString name, QVariant value) const
{
    if(name == OBJECT_VALUE_DISPLAY_NAME)
        return m_value.value("device_name");
    else
        return value;
}
...
void CDevice::afterSetStatus(short status, unsigned char value)
{
    ...
    qlonglong n = this->value(DEVICE_VALUE_ITEM_POINTER).toLongLong();
    if(!n)
        return;
...
{
    m_loop = NULL;
    setValue(OBJECT_VALUE_PIXMAP, ":/images/zone.png");
}
...
int CDevice::saveDevice(CDBSqlite *db)
{
    ...
    QString cmd = QString("SELECT ID, Number, Type "
                          ...
                          ).arg(value(DEVICE_VALUE_TYPE_ID).toInt());
    ...
    setValue(DEVICE_VALUE_CHANGETYPE,0);
    setValue(DEVICE_VALUE_CHANGEADD,0);
    ...
    query.bindValue(":Address", value(DEVICE_VALUE_ADDR).toInt());
    query.bindValue(":LampTypeID", lampTypeId);
    if(value(DEVICE_VALUE_AREA).toString() == "")
        query.bindValue(":Area", " ");
    else
        query.bindValue(":Area", value(DEVICE_VALUE_AREA).toString());
    if(value(DEVICE_VALUE_LOCATION).toString() == "")
        query.bindValue(":Location", " ");
    else
        query.bindValue(":Location", value(DEVICE_VALUE_LOCATION).toString());
    query.bindValue(":remark", value(DEVICE_VALUE_DESCRIPTION).toString());
    QString isReverse = value(DEVICE_VALUE_ISREVERSE).toString();
    if(isReverse == NULL)
        isReverse = "0";
