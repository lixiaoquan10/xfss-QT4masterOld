...
public:
    ...
    bool addController(CController* controller);
    void removeController(CController* controller);
    CController* controller(const int index) const { return m_controllers.value(index, NULL); }
    CController* controllerById(const int id) const;
    int controllerCount() const { return m_controllers.size(); }
    QList<CController*> controllers() const { return m_controllers; }
    bool hasController(const int id) const;
    int maxControllerId() const { return m_maxControllerId; }
    void calcMaxControllerId(const int n) { m_maxControllerId = m_maxControllerId > n ? m_maxControllerId : n; }
...
private:
    ...
    void addDeviceTypeAndOnline(CController* controller);
    void removeDeviceTypeAndOnline(CController* controller);
public:
