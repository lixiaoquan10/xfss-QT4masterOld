    ...
    CObject(keyId), m_onlineDistributionCount(0), m_onlinePowerCount(0), m_deviceOnlineTotal(0), m_onlineTotalCount(0)
{
    ...
    setValue(CONTROLLER_VALUE_NAME, value(CONTROLLER_VALUE_NAME).toString().append(QString::number(n)));
}
...
int CController::loadController(CDBSqlite *db, CProject *project)
{
    ...
    controller->setValue(CONTROLLER_VALUE_ID,63);
    controller->setValue(CONTROLLER_VALUE_PORT, 1);
    controller->setValue(CONTROLLER_VALUE_ADDRESS, 63);
    controller->setValue(CONTROLLER_VALUE_AREA, "");
    controller->setValue(CONTROLLER_VALUE_LOCATION, "");
...
QVariant CController::afterGetValue(QString name, QVariant value) const
{
    if(name == OBJECT_VALUE_DISPLAY_NAME)
        ...
        return m_value.value(CONTROLLER_VALUE_ID);
    else if(name == OBJECT_VALUE_DESCRIPTION)
        return m_value.value(CONTROLLER_VALUE_DESCRIPTION);
