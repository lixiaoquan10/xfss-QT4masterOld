...
public:
    ...
    QString descriptionStr() const { return value(LOOP_VALUE_DESCRIPTION).toString(); }

    bool setLoopId(int id) { return setValue(LOOP_VALUE_ID, id); }
    int loopId() const { return value(LOOP_VALUE_ID).toInt(); }
    int loopAdd() const { return value(LOOP_VALUE_ADDR).toInt(); }
