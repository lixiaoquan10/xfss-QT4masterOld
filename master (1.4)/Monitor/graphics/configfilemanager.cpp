    ...
    while(!childnode.isNull())
    {
        if(childnode.toElement().tagName() == "Distribution")
        {
            ...
            if(distribution)
            {
                ...
                distribution->setValue(DISTRIBUTION_VALUE_NAME,childnode.toElement().attribute("name"));
                distribution->setValue(DISTRIBUTION_VALUE_TYPEID,childnode.toElement().attribute("TypeId"));
                distribution->setValue(DISTRIBUTION_VALUE_DESCRIPTION,childnode.toElement().attribute("productID"));
...
CDistribution* ConfigFileManager::ParseBRANCH_LEVEL3(QDomNode *node, CDistribution* distribution)//解析三级分支
{
    ...
    while(!childnode.isNull())
    {
        if(childnode.toElement().tagName() == "Loop")
        {
            ...
            loop->setValue(LOOP_VALUE_ADDR,address);
            loop->setValue(LOOP_VALUE_ID,address);
            loop->setValue(LOOP_VALUE_NAME,childnode.toElement().attribute("name"));
...
CLoop* ConfigFileManager::ParseBRANCH_LEVEL4(QDomNode *node, CLoop* loop)//解析四级分支
{
    ...
    while(!childnode.isNull())
    {
        ...
        if(childnode.toElement().tagName() == "Device")
        {
            ...
            device->setValue(DEVICE_VALUE_DESCRIPTION,childnode.toElement().attribute("productID"));
            device->setValue(DEVICE_VALUE_ADDR,childnode.toElement().attribute("address"));
            device->setValue(DEVICE_VALUE_TYPE_ID,childnode.toElement().attribute("typeID"));
...
//            if(distribution && distribution->value(DISTRIBUTION_VALUE_DATABASE) == 0)
            if(distribution)
...
//            if(device && device->value(DEVICE_VALUE_DATABASE) == 0)
            //仅注册或者配置且注册的灯具  才保存到注册表
            if(device && (device->value(DEVICE_VALUE_ISUSED) == 1 || device->value(DEVICE_VALUE_ISUSED) == 2))
