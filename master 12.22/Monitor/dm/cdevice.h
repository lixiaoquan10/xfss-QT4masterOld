...
#define DEVICE_VALUE_ISUSED     "device_isused"
...
public:
    ...
    QString descriptionStr() const { return value(DEVICE_VALUE_DESCRIPTION).toString(); }
    bool isValueContain(QString value);

    void calcSecondCode();

    bool setDeviceId(int id) { return setValue(DEVICE_VALUE_ID, id); }
    int deviceId() const { return value(DEVICE_VALUE_ID).toInt(); }
    int deviceAdd() const { return value(DEVICE_VALUE_ADDR).toInt(); }
    int deviceTypeId() const { return value(DEVICE_VALUE_TYPE_ID).toInt(); }
    QString deviceProductionId() const { return value(DEVICE_VALUE_DESCRIPTION).toString(); }
    bool setSecondCode(QString& secondCode);
    QString secondCode() const { return value(DEVICE_VALUE_SECONDCODE).toString(); }
    bool setDeviceType(int type) { return setValue(DEVICE_VALUE_TYPE, type); }
    int deviceType() const { return value(DEVICE_VALUE_TYPE).toInt(); }
    void setLoop(CLoop* loop);
    CLoop* loop() const { return m_loop; }

    QString DeviceEmergency() const { return value(DEVICE_VALUE_EMERGENCY).toString(); }
    QString DeviceLight() const { return value(DEVICE_VALUE_LIGHT).toString(); }
