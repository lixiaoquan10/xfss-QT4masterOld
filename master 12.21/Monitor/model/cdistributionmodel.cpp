...
CDistributionModel::CDistributionModel(CController* controller, bool track, QObject *parent) :
    CModel(parent), m_controller(0)
{
    setTrack(track);
...
void CDistributionModel::setController(CController *controller)
{
    if(m_controller != controller)
    {
        ...
        if(m_controller)
        {
            ...
            QList<CDistribution*> distributions = m_controller->distributions();
            for(int i = 0; i < distributions.size(); ++i)
            {
                loadChildren(distributions.at(i));
            }
            distributions.clear();
        }
    }
...
//QVariant CDistributionModel::getDisplayData(const QModelIndex &index) const
//{
...
//        return data->value(DISTRIBUTION_VALUE_NAME);
...
//        return data->value(DISTRIBUTION_VALUE_AREA);
//    else if(index.column() == DISTRIBUTION_COLUMN_LOCATION)
//        return data->value(DISTRIBUTION_VALUE_LOCATION);
//    return QVariant();
//}
...
//            return false;
////        if(controller->hasDistribution(id))
////        {
...
//        if(name.isEmpty())
//            return false;
//        distribution->setValue(DISTRIBUTION_VALUE_NAME, name);
//        break;
//    }
//    case DISTRIBUTION_COLUMN_DESCRIPTION:
//    {
//        distribution->setValue(DISTRIBUTION_VALUE_DESCRIPTION, value);
//        break;
//    }
