...
void CStatusTabs::slot_tableviewClicked(const QModelIndex &index)
{
    ...
    if(distribution)
    {
        ...
        if(loop)
        {
            ...
            if(device)
            {
                int layoutID = device->value(DEVICE_VALUE_LAYOUTPAGEID).toInt();
                if(!device->isValueContain(DEVICE_VALUE_CX) || !device->isValueContain(DEVICE_VALUE_CY))
                    return;
                double valueX = device->value(DEVICE_VALUE_CX).toDouble();
                double valueY = device->value(DEVICE_VALUE_CY).toDouble();
