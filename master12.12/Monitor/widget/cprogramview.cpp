...
void CProgramView::slot_regALL()
{
    ...
    if(ret == QMessageBox::Yes)
    {
         ...
         if(!CGlobal::instance()->ClientBusiness()->exeCommand(NCT_LoginInfo, &m_MsgNUll))
         {
             ...
             QMessageBox::information(this, "信息", "注册失败！");
...
void CProgramView::slot_unLogin()
{
    ...
    if(ret == QMessageBox::Yes)
    {
        ...
        {
            CGlobal::instance()->m_HaveDialog = true;
            QMessageBox::information(this, "信息", "清除注册失败！");
            CGlobal::instance()->m_HaveDialog = false;
        }
        else
        {
...
void CProgramView::slot_regLoop()
{
    ...
    {
        ret = QMessageBox::information(NULL, "信息",
                                 "确定要让指定的CAN设备单个注册吗?",
                                           QMessageBox::Yes, QMessageBox::Cancel);
    }
    ...
    if(ret == QMessageBox::Yes)
    {
        ...
        {
            CGlobal::instance()->m_HaveDialog = true;
            QMessageBox::information(this, "信息", "单个注册失败！");
            CGlobal::instance()->m_HaveDialog = false;
        } else {
...
void CProgramView::slot_unLoginLoop()
{
    ...
    if(ret == QMessageBox::Yes)
    {
        ...
        {
            CGlobal::instance()->m_HaveDialog = true;
            QMessageBox::information(this, "信息", "单个清除注册失败！");
            CGlobal::instance()->m_HaveDialog = false;
        }
        else
        {
                        ...
                        if(deviceAdd == devices.at(n)->deviceAdd())
                        {
                            ...
                            for(int z = 1; z<256; z++)
                            {
                                ...
                                if(!device)
                                {
                                    qDebug() << z;
                                    device = devices.at(n);
                                    device->setValue(DEVICE_VALUE_ADDR,z);
                                    device->setValue(DEVICE_VALUE_CHANGEADD,1);
                        ...
                        if(deviceAdd == devices.at(n)->deviceAdd())
                        {
                            ...
                            for(int i = 1; i<256; i++)
                            {
                                ...
                                if(!device)
                                {
                                    ...
                                    qDebug() << i;
                                    device = devices.at(n);
                                    device->setValue(DEVICE_VALUE_ADDR,i);
                                    device->setValue(DEVICE_VALUE_CHANGEADD,1);
                                    CMsgObjectStatus msgobjectstatus;
                    ...
                    for(int n = m+1; n<devices.count(); n++)
                    {
                        if(deviceAdd == devices.at(n)->deviceAdd())
                        {
                            ...
                            QMessageBox::information(NULL, "信息",
                                                     tr("集中电源%1").arg(distribution->distributionAddress())
                                                     + tr("回路%2").arg(loop->loopAdd()) + "存在重码", QMessageBox::Ok);
                            reflag = true;
                            return;
                        }
                    }
            ...
            if(loop){
                ...
                for(int m=0; m<devices.count(); m++){
                    ...
                    for(int n = m+1; n<devices.count(); n++)
                    {
                        ...
                        {
                            qDebug() << m << n << deviceAdd;
                            QMessageBox::information(NULL, "信息", "当前回路存在重码", QMessageBox::Ok);
                            return;
                        }
                    }
                }
                int ret = QMessageBox::information(NULL, "信息",
                                         "是否从数据库中删除该回路未注册到的灯具?" + tr("\n") + "Yes:是" + tr("\n") + "No:否" + tr("\n") + "Cancel:取消",
                                                   QMessageBox::Yes, QMessageBox::No, QMessageBox::Cancel);
                if(ret == QMessageBox::Cancel)
                {
                    return;
                ...
                if(!db.open())
                {
                    QMessageBox::information(NULL, "信息", "保存失败！", QMessageBox::Ok);
                    return;
                }
                else
                {
                    ...
                    {
                        QMessageBox::information(NULL, "信息", "保存成功，请重启软件！(主菜单中“系统”->“重启”)", QMessageBox::Ok);
                    }
                }
                db.close();
                CGlobal::instance()->m_isDelUnregLampFlag = false;
