...
//    colorList.append("background-color:yellow");
    colorList.append("background-color:green");
//    colorList.append("background-color:cyan");
    colorList.append("background-color:blue");
//    colorList.append("background-color:magenta");
 //   colorList.append("background-color:black");
}
...
void CDataProcessing::slotNotifyInformation(int type,QList<QVariant> data)
{
    ...
    switch(type)
    {
        ...
        case NIT_CheckTimeUpdate:
            ...
            break;
        case NIT_FASCommunicationFault:
            fasCommunication(data);
            break;
        case NIT_HasFASSystem:
            hasFasSystem(data);
            break;
...
void CDataProcessing::initResult(QList<QVariant> data)
{
    ...
    if(data.at(0).toInt() == 0)//初始化成功----------------------------------
    {
        ...
        if (m_isFas) {
            CGlobal::instance()->setMsgText("FAS系统通讯正常！");
        } else {
            CGlobal::instance()->setMsgText("系统运行！");
        }

        QList<CProject*> tempProject;//获取工程列表
        tempProject = CGlobal::instance()->dm()->projects();

        QList<CLayer *> layerList = CGlobal::instance()->getLayers(tempProject);//获取图层链
    ...
    else if(data.at(0).toInt() == -1)//初始化失败----------------------------------
    {
        ...
        QMessageBox::information(NULL, "信息", "加载文件失败！");
...
void CDataProcessing::resetSystem(QList<QVariant> data)
{
    ...
    if(!CGlobal::instance()->m_bInLogin)
        CGlobal::instance()->InformationWindow()->hide();
    QTimer::singleShot(2000, CGlobal::instance()->ClientBusiness(), SLOT(checkUnRegDevice()));
}
...
void CDataProcessing::slotcheckLoginInfo()
{
    ...
    CController* checkController = new CController(63);
    ...
    if(controller){
        ...
        for(int i=0; i<list.count(); i++){
            CDistribution* distribution = list.at(i);
            if(distribution){
                checkController = CGlobal::instance()->ClientBusiness()->controller();
                if(checkController)
                    checkDistribution = checkController->distributionByAddress(distribution->distributionAddress());
                ...
                {
                    distribution->setValue(DISTRIBUTION_VALUE_TYPEID,checkDistribution->distributionTypeId());
                    distribution->setValue(DISTRIBUTION_VALUE_DESCRIPTION,checkDistribution->descriptionStr());
                    distribution->setValue(DISTRIBUTION_VALUE_NAME,checkDistribution->typeStr());
                    ...
                    if(loop){
                        ...
                        for(int m=0; m<devices.count(); m++){
                            ...
                            if(device)
                            {
                                ...
                                if(!checkDevice || !checkDistribution || (checkDistribution && !checkLoop))
                                {
                                    device->setStatus(OBJS_Online,0);
                                    device->setStatus(OBJS_LightCommunicationFault,1);
                                    //上传灯具状态到云平台/客户端
...
void CDataProcessing::fasCommunication(QList<QVariant> data)
{
    bool flag = data.at(0).toBool();
    if (flag && m_isFas)
    {
        CGlobal::instance()->setMSgColor(Qt::red);
        CGlobal::instance()->setMsgText("FAS系统通讯失败！");
    }
    else if (m_isFas)
    {
        CGlobal::instance()->setMSgColor(QColor(12, 180, 3));
...
void CDataProcessing::hasFasSystem(QList<QVariant> data)
{
    Q_UNUSED(data);
    m_isFas = true;
    CGlobal::instance()->setMsgText("FAS系统通讯正常！");
    ...
    if(CGlobal::instance()->m_UserType == CGlobal::User)
    {
        ...
        if(flag)
        {
            ...
            if(QDialog::Accepted == m_Administrator.exec())
            {
                ...
                {
                    CGlobal::instance()->m_HaveDialog = true;
                    QMessageBox::information(NULL, "信息", "操作失败！");
                    CGlobal::instance()->m_HaveDialog = false;
                }
                delete msgData2;
            }
