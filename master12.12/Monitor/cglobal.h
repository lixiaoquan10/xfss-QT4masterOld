...
//    int LampTypeAnalysis(int deviceTypeId);
    ...
    const QList<CLayer *> getLayers(const QList<CProject *> &projectList);
    ...
    CProgramProjectView* programProjectView() const { return m_pProgramProjectView; }
    void setProgramProjectView(CProgramProjectView* programProjectView) { m_pProgramProjectView = programProjectView; }
...
private:
    ...
    CProgramProjectView* m_pProgramProjectView;
...
public:
    ...
    int m_nDistriAddress;          //当前选中的集中电源地址
    int m_nLoopAddress;            //当前选中的回路地址
    bool m_isSelectDistribution;   //当前选中的是集中电源
    bool m_isSelectLoop;           //当前选中的是回路
    bool m_isDelUnregLampFlag;     //是否删除未注册到的灯具标志
