...
void CProcessController::mute(bool enable)
{
    ...
    if(m_isFault && m_isStartStatus)
        enable = false;
    m_isMute = enable;
//    CGlobal::instance()->processClient()->mute(enable);
}
//设备复位
void CProcessController::manualDeviceReset(CMsgDeviceInfo *msgDeviceInfo)
{
    ...
    int deviceAddress = msgDeviceInfo->deviceInfo.value(DI_DeviceId, 0);
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->lightReset(distributionAddress, loopAddress, deviceAddress);
...
//        if(m_isEmergency == enable) return;
        m_isEmergency = enable;
    } else if(emergencyType == OBJS_RemoteEmergency){
//        if(m_isRemoteEmergency == enable) return;
        m_isRemoteEmergency = enable;
    } else if(emergencyType == OBJS_FireEmergency){
//        if(m_isFireEmergency == enable) return;
        m_isFireEmergency = enable;
    } else if(emergencyType == OBJS_InputEmergency){
//        if(m_isInputEmergency == enable) return;
        m_isInputEmergency = enable;
    } else if(emergencyType == OBJS_ForceEmergency){
//        if(m_isForceEmergency == enable) return;
        m_isForceEmergency = enable;
    }
//    QString str = enable?tr("System ForceEmergency Start"):tr("System ForceEmergency Stop");
//    CGlobal::instance()->print()->print(CPrint::Other, str);
...
void CProcessController::sendMonthCheck(bool enable)
{
    ...
    if(processDistribution)
    {
        processDistribution->monthCheck(enable);
    }
}
...
void CProcessController::sendYearCheck(bool enable)
{
    ...
    if(processDistribution)
    {
        processDistribution->yearCheck(enable);
    }
}
...
//    if(!m_checkTime->isEntryCheckTimer)// 如果没有进入检测计时状态（持续主电工作不足48小时/2天）
//    {
//        if(m_isMainPowerFault)// 如果存在主电故障，重新计算开始计时时间
//        {
//            m_checkTime->systemMainPowerTime = m_checkTime->systemCurrentTime;
//        }
//        else if(m_checkTime->systemMainPowerTime.addDays(2) <= m_checkTime->systemCurrentTime)
//        {
//            m_checkTime->isEntryCheckTimer = true;// 如果存在主电故障，重新计算开始计时时间
//        }
//    }
...
//全部注册
void CProcessController::loginInfo()
{
    CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
    if(processDistribution)
        processDistribution->reLogin();
}
//全部清除注册
void CProcessController::unLogin()
{
    CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
    if(processDistribution)
        processDistribution->unLogin();
}
//灯具修改地址
void CProcessController::DeviceAdd(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    QString strText = msgObjectStatus->strText;
    int value = msgObjectStatus->nValue;
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0 && value != deviceAddress)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->DeviceAdd(distributionAddress, loopAddress, deviceAddress, value, strText);
    }
}
//灯具修改类型
void CProcessController::DeviceType(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    ...
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0 && value != typeId)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->DeviceType(distributionAddress, loopAddress, deviceAddress, value);
    }
}
//灯具应急
void CProcessController::manualDeviceEmergency(CMsgDeviceInfo *msgDeviceInfo, bool enable)
{
    int distributionAddress = msgDeviceInfo->deviceInfo.value(DI_DistributionId, 0);
    int loopAddress = msgDeviceInfo->deviceInfo.value(DI_LoopId, 0);
    int deviceAddress = msgDeviceInfo->deviceInfo.value(DI_DeviceId, 0);
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->lightControl(enable, distributionAddress, loopAddress, deviceAddress);
    }
}
//灯具测试
void CProcessController::manualDeviceTest(CMsgDeviceInfo *msgDeviceInfo)
{
    int distributionAddress = msgDeviceInfo->deviceInfo.value(DI_DistributionId, 0);
    int loopAddress = msgDeviceInfo->deviceInfo.value(DI_LoopId, 0);
    int deviceAddress = msgDeviceInfo->deviceInfo.value(DI_DeviceId, 0);
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->lightTest(distributionAddress, loopAddress, deviceAddress);
    }
}
//灯具默认点亮
void CProcessController::DeviceDefaultLighten(CMsgDeviceInfo *msgDeviceInfo, bool enable)
{
    int distributionAddress = msgDeviceInfo->deviceInfo.value(DI_DistributionId, 0);
    int loopAddress = msgDeviceInfo->deviceInfo.value(DI_LoopId, 0);
    int deviceAddress = msgDeviceInfo->deviceInfo.value(DI_DeviceId, 0);
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->lampdefaultlighten(enable, distributionAddress, loopAddress, deviceAddress);
    }
}

//标志灯默认方向
void CProcessController::DeviceDefaultDirection(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    ...
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->lampdefaultdirection(value, distributionAddress, loopAddress, deviceAddress);
    }
}
//标志灯方向控制
void CProcessController::DeviceDirectionControl(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    ...
    if(distributionAddress != 0 && loopAddress != 0 && deviceAddress != 0)
    {
        CProcessDistribution* processDistribution = CGlobal::instance()->processDistribution();
        if(processDistribution)
            processDistribution->directionControl(distributionAddress, loopAddress, deviceAddress, value);
    }
}

//标志灯闪灯配置
void CProcessController::DeviceFlashConfig(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    ...
    if(processDistribution)
    {
        processDistribution->flashConfig(value, distributionAddress, loopAddress, deviceAddress);
    }
}

//标志灯闪灯控置
void CProcessController::DeviceFlashControl(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    ...
    if(processDistribution)
    {
        processDistribution->flashControl(value, distributionAddress, loopAddress, deviceAddress);
    }
}

//可变状态灯设置
void CProcessController::ChangeControl(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    int value = msgObjectStatus->nValue;
    CGlobal::instance()->processDistribution()->ChangeControl(value, distributionAddress, loopAddress, deviceAddress);
}

//自带电池灯具关闭备电输出
void CProcessController::CloseBatteryOut(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int deviceAddress = msgObjectStatus->nDeviceID;
    int value = msgObjectStatus->nValue;
    CGlobal::instance()->processDistribution()->CloseBatteryOut(value, distributionAddress, loopAddress, deviceAddress);
}

//配电箱应急回路输出控制
void CProcessController::SetLoopOutStatus(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    int value = msgObjectStatus->nValue;
    CGlobal::instance()->processDistribution()->SetLoopOutStatus(value, distributionAddress, loopAddress);
}

//读取配电箱应急回路输出状态
void CProcessController::ReadLoopOutStatus(CMsgObjectStatus *msgObjectStatus)
{
    int distributionAddress = msgObjectStatus->nDisID;
    int loopAddress = msgObjectStatus->nLoopID;
    CGlobal::instance()->processDistribution()->ReadLoopOutStatus(distributionAddress, loopAddress);
}
...
void CProcessController::RecordPage(CMsgObjectStatus *msgObjectStatus)
{
    ...
    bool m_isStatusTab = msgObjectStatus->ndata.at(1);
    int page = msgObjectStatus->ndata.at(2);
    //查询模式：1为自定义查询，有故障或应急时显示5条记录，否则显示x条;0为普通查询，有故障或应急时显示6条记录，否则显示x条
    if(customMode)
    {
        if(m_isStatusTab)
            CGlobal::instance()->setRecordNumber(5);
        else
            CGlobal::instance()->setRecordNumber(9);
    }
    else
    {
        if(m_isStatusTab)
            CGlobal::instance()->setRecordNumber(6);
        else
            CGlobal::instance()->setRecordNumber(10);
    }
