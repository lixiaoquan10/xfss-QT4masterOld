...
public:
    ...
    CController* loginController() const { return m_controller; }
    void setLoginController(CController* controller) { m_controller = controller; }

    void showTest(bool bwrite);
    void InitProject();
    void clearExpectObject();
    ...
    void removeLoginObject(int distributionAdd = 0, int loopAdd = 0);
    void setNewPwd(const QString strpwd);
    CController* currentPanel() const;
    void writeDebug(QString strmsg = "");
    void ForceEmergency();
    void InputEmergency();
    void Emergency();
    void DeviceResetStatus();
    void DeviceStartStatus(int distributionAdd, int loopAdd, int deviceAdd, QString directionStatus, QString directionAndTwinkleStatus, QDateTime time);
...
private:
    void controllerStatusChanged(CController* controller, short status, unsigned char value);
    bool hasStandbyPowerFault(CController* controller) const;
    void createPath(const QString& path) const;

    void closeProject();
    void clearProjectFile();
    void reset();    
    CController* createUndefinedController(CProject* project, int address, int keyId=0);
    CDistribution *createUndefinedDistribution(CController *controller, int address , QString str = "", int keyId = 0);
    CLoop* createUndefinedLoop(CDistribution *distribution, int address, int keyId=0);
    CLoop* createUndefinedLoop(CController *controller, int address, int keyId=0);
...
private:
    ...
    CMsgControllerInfo m_controllerInfo;
...
//class CClientBusinessWrapper : public QObject
//{
//    Q_OBJECT
//public:
//    explicit CClientBusinessWrapper(CClientBusiness* clientBusiness, QObject *parent = NULL)
//        : QObject(parent), m_clientBusiness(clientBusiness)
//    {}
