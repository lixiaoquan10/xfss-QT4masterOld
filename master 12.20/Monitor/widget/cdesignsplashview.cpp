    ...
    QWidget(parent)
{
    ...
    m_enlarge->setMaximumHeight(35);
    m_enlarge->setMaximumWidth(35);
    connect(m_enlarge,SIGNAL(clicked(bool)),this,SLOT(slot_enlarge()));
    m_verticalSlider = new QSlider();
    ...
    m_reduction->setMaximumHeight(35);
    m_reduction->setMaximumWidth(35);
    connect(m_reduction,SIGNAL(clicked(bool)),this,SLOT(slot_reduction()));
    pLeftLayout->addWidget(m_enlarge);
    pLeftLayout->addWidget(m_verticalSlider);
...
void CDesignSplashView::slot_setPrimitiveItem()
{
    ...
    if(!CGlobal::instance()->designTreeView()->CurrentLayer())
        return;
    int layoutPageID = CGlobal::instance()->designTreeView()->CurrentLayer()->value(LAYER_VALUE_ID).toInt();
...
void CDesignSplashView::setPrimitiveItem(int layerID)
{
    ...
    if(!CGlobal::instance()->designTreeView()->CurrentLayer())
        return;
    int currentlayoutPageID = CGlobal::instance()->designTreeView()->CurrentLayer()->value(LAYER_VALUE_ID).toInt();
    // 加载main.png
    QString pictureName = CGlobal::instance()->designTreeView()->currentLayer()->value(LAYER_VALUE_BACKGROUNDPICTURE).toString();
    QString filePath = "/home/xfss/root/PageBackgroundFolder/" + pictureName;
    ...
    if(!db.open())
        return;
    QString str = QString("SELECT l.ID,l.LampInfoID,l.LayoutPageID,l.CX,l.CY,l.Height,l.Angle,m.Number,r.Address,c.Loop,a.Address from Coordinate l "
    ...
    while(query.next())
    {
        //8:r.Address灯具地址    9:c.Loop回路地址   10:a.Address集中电源地址
        CDistribution* distribution = controller->distributionByAddress(query.value(10).toInt());
        if(!distribution)
            continue;
...
//        int LampInfoID = query.value(fieldLampInfoID).toInt();
        ...
        device->setValue(DEVICE_VALUE_CX,CX);
        device->setValue(DEVICE_VALUE_CY,CY);
        device->setValue(DEVICE_VALUE_HEIGHT,Height);
        device->setValue(DEVICE_VALUE_ANGEL,Angle);
        device->setValue(DEVICE_VALUE_LAYOUTPAGEID,LayoutPageID);
        //双向灯
    ...
    {
        QString string = QString("SELECT l.ID,l.StartLampID,l.Joint,l.EndLampID,r1.Address,c1.Loop,a1.Address,r2.Address,c2.Loop,a2.Address from LampConnection l "
        ...
        {
            CDistribution* distribution1 = controller->distributionByAddress(queryConnection.value(6).toInt());
            if(!distribution1)
                continue;
            ...
            CDistribution* distribution2 = controller->distributionByAddress(queryConnection.value(9).toInt());
            if(!distribution2)
                continue;
            CLoop* loop2 = distribution2->loopByAdd(queryConnection.value(8).toInt());
            if(!loop2)
                continue;
            EndLamp = loop2->deviceByAdd(queryConnection.value(7).toInt());
            if(StartLamp->value(DEVICE_VALUE_LAYOUTPAGEID).toInt() == currentlayoutPageID)
...
void CDesignSplashView::mousePressEvent(QMouseEvent* event) {
    ...
    if(!CGlobal::instance()->designTreeView()->CurrentLayer())
        return;
    //当前页面
    int pageID = CGlobal::instance()->designTreeView()->currentLayer()->value(LAYER_VALUE_ID).toInt();
    // 获取鼠标点击位置
                ...
                if(loop){
                    ...
                    for(int i=0; i<devices.count(); i++){
                        ...
                        if(device)
                        {
                            ...
                            double angel = device->value(DEVICE_VALUE_ANGEL).toDouble();
                            double scale = device->value(DEVICE_VALUE_HEIGHT).toDouble()/40;
                            double device_x1 = device->value(DEVICE_VALUE_CX).toDouble();
                            double device_y1 = device->value(DEVICE_VALUE_CY).toDouble();
                            ...
                            if(device->value(DEVICE_VALUE_LAYOUTPAGEID).toInt() != pageID)
                                continue;
                            //点击的位置在灯具区域内
...
QMenu* CDesignSplashView::createTableMenu(CDevice* device)
{
    ...
    //    }
                ...
                typeID == 121 || typeID == 123)
        {
            ...
            directionMenu->addAction("右向", this, SLOT(slot_Right()));
            tableMenu->addMenu(directionMenu);
            directionMenu->addAction("全亮", this, SLOT(slot_AllBright()));
            tableMenu->addMenu(directionMenu);
            directionMenu->addAction("全灭", this, SLOT(slot_AllClose()));
            directionMenu->addSeparator();
    ...
    if(typeID == 121 || typeID == 123)
    {
        ...
        if(m_device->value(DEVICE_VALUE_DEFAULTDIRECTION) == 2)
        {
            m_left->setIcon(QIcon(":/images/ok.png"));
        }
        else if(m_device->value(DEVICE_VALUE_DEFAULTDIRECTION) == 1)
        {
            m_right->setIcon(QIcon(":/images/ok.png"));
        }
        else if(m_device->value(DEVICE_VALUE_DEFAULTDIRECTION) == 0)
        {
            m_all->setIcon(QIcon(":/images/ok.png"));
        }
        else if(m_device->value(DEVICE_VALUE_DEFAULTDIRECTION) == 3)
        {
        ...
        if(device->value(DEVICE_VALUE_ISREVERSE).toInt() == 1)
        {
            m_yes->setIcon(QIcon(":/images/ok.png"));
        }
        else if(device->value(DEVICE_VALUE_ISREVERSE).toInt() == 0)
        {
...
{
    double startX = startLamp->value(DEVICE_VALUE_CX).toDouble();
    double startY = startLamp->value(DEVICE_VALUE_CY).toDouble();
    double endX = endLamp->value(DEVICE_VALUE_CX).toDouble();
    double endY = endLamp->value(DEVICE_VALUE_CY).toDouble();
    double startHeight = startLamp->value(DEVICE_VALUE_HEIGHT).toDouble();
    double endHeight = endLamp->value(DEVICE_VALUE_HEIGHT).toDouble();
    double startAngle = startLamp->value(DEVICE_VALUE_ANGEL).toDouble();
    double endAngle = endLamp->value(DEVICE_VALUE_ANGEL).toDouble();
...
//    painter.scale(startHeight/40, startHeight/40);
    ...
    if(((startLamp->value(DEVICE_VALUE_LEFTCLOSE) == 1) && (joint == 1)) ||
        ((startLamp->value(DEVICE_VALUE_RIGHTCLOSE) == 1) && (joint == 2)))
...
void CDesignSplashView::setArrowPrimitiveItem(QPixmap* background, CDevice* device)
{
    ...
    double x = device->value(DEVICE_VALUE_CX).toDouble();
    double y = device->value(DEVICE_VALUE_CY).toDouble();
    double height = device->value(DEVICE_VALUE_HEIGHT).toDouble();
    qreal rotation = device->value(DEVICE_VALUE_ANGEL).toDouble();
    ...
    //应急状态，设置填充颜色为Red
    else if(device->value(DEVICE_VALUE_EMERGENCY) == "应急" || CGlobal::instance()->m_isEmergency == true)
        painter.setBrush(QBrush(QColor(255,0,0)));
    //光源故障，设置填充颜色为Yellow1
    else if(device->value(DEVICE_VALUE_LIGHT) == "光源故障")
    ...
    int leftClose = device->value(DEVICE_VALUE_LEFTCLOSE).toInt();
    int rightClose = device->value(DEVICE_VALUE_RIGHTCLOSE).toInt();
    int defaultDirection = device->value(DEVICE_VALUE_DEFAULTDIRECTION).toInt();
    QString emergency = device->value(DEVICE_VALUE_EMERGENCY).toString();
    // 绘制左箭头
...
void CDesignSplashView::slot_Lighten()
{
    ...
    msgDeviceInfo.deviceInfo.insert(DI_KeyId, m_device->keyId());
    msgDeviceInfo.deviceInfo.insert(DI_DeviceId, m_device->deviceAdd());
    msgDeviceInfo.deviceInfo.insert(DI_LoopId, m_device->value(DEVICE_VALUE_LOOPADDR).toInt());
    msgDeviceInfo.deviceInfo.insert(DI_DistributionId, m_device->value(DEVICE_VALUE_DISTRIBUTIONADDR).toInt());

    CGlobal::instance()->ClientBusiness()->exeCommand(NCT_DeviceDefaultOn, &msgDeviceInfo);
...
void CDesignSplashView::slot_CloseLight()
{
    ...
    CGlobal::instance()->ClientBusiness()->exeCommand(NCT_DeviceDefaultOff, &msgDeviceInfo);
...
void CDesignSplashView::slot_Left()
{
    ...
    if(!m_device)
        return;
    m_device->setValue(DEVICE_VALUE_LEFTCLOSE,0);
    m_device->setValue(DEVICE_VALUE_RIGHTCLOSE,1);
    CMsgObjectStatus msgobjectstatus;
    msgobjectstatus.nDisID = m_device->value(DEVICE_VALUE_DISTRIBUTIONADDR).toInt();
    msgobjectstatus.nLoopID = m_device->value(DEVICE_VALUE_LOOPADDR).toInt();
...
void CDesignSplashView::slot_Right()
{
    ...
    if(!m_device)
        return;
    m_device->setValue(DEVICE_VALUE_LEFTCLOSE,1);
    m_device->setValue(DEVICE_VALUE_RIGHTCLOSE,0);
    CMsgObjectStatus msgobjectstatus;
    msgobjectstatus.nDisID = m_device->value(DEVICE_VALUE_DISTRIBUTIONADDR).toInt();
    msgobjectstatus.nLoopID = m_device->value(DEVICE_VALUE_LOOPADDR).toInt();
    msgobjectstatus.nDeviceID = m_device->deviceAdd();
...
void CDesignSplashView::slot_DefaultLeft()
{
    ...
    m_device->setValue(DEVICE_VALUE_DEFAULTDIRECTION,2);
    ...
    if(!query.exec())
    {
        ...
        QMessageBox::information(this,"错误","灯具:" + QString::number(msgobjectstatus.nDisID) + tr("-") + QString::number(msgobjectstatus.nLoopID) + tr("-") +
...
void CDesignSplashView::slot_DefaultRight()
{
    ...
    CGlobal::instance()->ClientBusiness()->exeCommand(NCT_DeviceDefaultDrection, &msgobjectstatus);
    m_device->setValue(DEVICE_VALUE_DEFAULTDIRECTION,1);
...
void CDesignSplashView::slot_DefaultAll()
{
    ...
    CGlobal::instance()->ClientBusiness()->exeCommand(NCT_DeviceDefaultDrection, &msgobjectstatus);
    m_device->setValue(DEVICE_VALUE_DEFAULTDIRECTION,0);
...
void CDesignSplashView::slot_DefaultOff()
{
    ...
    CGlobal::instance()->ClientBusiness()->exeCommand(NCT_DeviceDefaultDrection, &msgobjectstatus);
    m_device->setValue(DEVICE_VALUE_DEFAULTDIRECTION,3);
...
{
    if(m_device->value(DEVICE_VALUE_ISREVERSE).toInt() == 1)
        return;
    m_device->setValue(DEVICE_VALUE_ISREVERSE,1);
    QString fileName = CGlobal::instance()->workspaces() + "/" + "ESSQLiteCE100.db";
    CDBSqlite db(fileName);
    if(!db.open())
        return;
    ...
    QString cmd = QString("SELECT ID, Number, Type "
                          ...
                          ).arg(m_device->value(DEVICE_VALUE_TYPE_ID).toInt());
    ...
    if(m_device->value(DEVICE_VALUE_AREA).toString() == "")
        query.bindValue(":Area", " ");
    else
        query.bindValue(":Area", m_device->value(DEVICE_VALUE_AREA).toString());
    if(m_device->value(DEVICE_VALUE_LOCATION).toString() == "")
        query.bindValue(":Location", " ");
    else
        query.bindValue(":Location", m_device->value(DEVICE_VALUE_LOCATION).toString());
    query.bindValue(":remark", m_device->value(DEVICE_VALUE_DESCRIPTION).toString());
    query.bindValue(":isreverse", m_device->value(DEVICE_VALUE_ISREVERSE).toString());
    query.bindValue(":isused", m_device->value(DEVICE_VALUE_ISUSED));

    if(!query.exec())
    {
        qDebug()<<query.lastError().text();
        QMessageBox::information(this,"错误", tr("反接保存数据库失败！"));
    }
    db.close();
}
//双向地埋灯未反接
...
{
    if(m_device->value(DEVICE_VALUE_ISREVERSE).toInt() == 0)
        return;
    m_device->setValue(DEVICE_VALUE_ISREVERSE,0);
    QString fileName = CGlobal::instance()->workspaces() + "/" + "ESSQLiteCE100.db";
    CDBSqlite db(fileName);
    if(!db.open())
        return;
...
void CDesignSplashView::slot_StartTest()
{
    ...
    m_device->setValue(DEVICE_VALUE_TEST,"测试");
...
void CDesignSplashView::slot_StopTest()
{
    ...
    m_device->setValue(DEVICE_VALUE_TEST,"正常");
    m_device->setValue(DEVICE_VALUE_EMERGENCY,"正常");
...
void CDesignSplashView::slot_StartEmergency()
{
    if(!m_device)
        return;
    if(m_device->getStatus(OBJS_LightCommunicationFault))
        return;
    CMsgDeviceInfo msgDeviceInfo;
    msgDeviceInfo.deviceInfo.insert(DI_KeyId, m_device->keyId());
    msgDeviceInfo.deviceInfo.insert(DI_DeviceId, m_device->deviceAdd());
    ...
    m_device->setValue(DEVICE_VALUE_EMERGENCY,"应急");
    CGlobal::instance()->DlgDevices()->getDeviceInfo();
...
void CDesignSplashView::slot_DeviceInfo()
{
    ...
    QMessageBox::information(this,"灯具信息",
                             QString::number(distribution->distributionAddress()) + tr("-") + QString::number(loop->loopAdd()) + tr("-") +
                             QString::number(m_device->deviceAdd()) + tr("\n") + m_device->value(DEVICE_VALUE_TYPE).toString() + tr("\n") +
                            "区域:" + m_device->value(DEVICE_VALUE_AREA).toString() + tr("\n") +
                            "位置:" + m_device->value(DEVICE_VALUE_LOCATION).toString());
