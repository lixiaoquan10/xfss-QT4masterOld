    ...
    if(m_pSerialPort && m_pSerialPort->isOpen()){
        ...
        if(true)
        {
            if (m_pSerialPort->isOpen()) {
                ...
                QString hexString = data.toHex(); // ½«×Ö½Ú ý×é×ª»»Îª ®Áù½øÖÆ×Ö·û´®
                // ÅÐ¶Ï·¢ËÍ Ç·ñ³É¹¦
...
//    QByteArray buffer; // »º³åÇø£¬ÓÃÓÚ´æ´¢¶ÁÈ¡µÄ ý¾Ý
...
//        } else {
////           buffer.clear(); // Çå¿Õ»º³åÇøÖÐµÄÎÞÐ§ ý¾Ý
//           break;
//        }
    }
...
//    }
    ...
    QString  data = "cancard1ReciveData:  " + bytes.toHex() + "   " + QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    /*??????????????????????*/
    QFile file("/home/xfss/root/receivedata.txt");
    ...
    if(m_sendCommandCacheTable.count() > 0){
        foreach(int key, m_sendCommandCacheTable.keys())
        {
            ...
            if(pData && pData->endTime<getTickCount())//³¬ ±
            {
                ...
                if(pData->repeatCount<1){//ÖØ·¢´Î ý³¬¹ý×î´óÖµ
                    delete pData;
