...
bool WmfParser::load(const QByteArray& array)
{
    ...
    if (pheader.key == (quint32)APMHEADER_KEY) {
        ...
        mPlaceable = true;
#if DEBUG_RECORDS
        qDebug() << "Placeable header!  Yessss!";
#endif
        ...
        if (pheader.checksum != checksum) {
            qWarning() << "Checksum for placeable metafile header is incorrect ( actual checksum" << pheader.checksum << ", expected checksum" << checksum << ")";
            return false;
        }
...
#if DEBUG_RECORDS
        qDebug() << "bounding box in header: " << mBBoxLeft << mBBoxTop << mBBoxRight << mBBoxBottom
    ...
    if (((header.headerSize == 9) && (header.numOfParameters == 0)) || (mPlaceable)) {
        ...
        qDebug() << "WmfParser : incorrect file format !";
    }
...
#if DEBUG_RECORDS
        qDebug() << "bounding box created by going through all records: "
...
{
    if (!(mValid)) {
        qDebug() << "WmfParser::play : invalid WMF file";
        return false;
    }
...
#if 1
        if ((mStandard)) {
            qDebug() << "Standard :" << mBBoxLeft << ""  << mBBoxTop << ""  << mBBoxRight - mBBoxLeft << ""  << mBBoxBottom - mBBoxTop;
        } else {
            qDebug() << "DPI :" << mDpi;
            qDebug() << "size (inch):" << (mBBoxRight - mBBoxLeft) / mDpi
                          << "" << (mBBoxBottom - mBBoxTop) / mDpi;
            qDebug() << "size (mm):" << (mBBoxRight - mBBoxLeft) * 25.4 / mDpi
                          << "" << (mBBoxBottom - mBBoxTop) * 25.4 / mDpi;
        }
        qDebug() << mValid << "" << mStandard << "" << mPlaceable;
#endif
    }
...
#if DEBUG_RECORDS
            qDebug() << "Record = " << koWmfFunc[ index ].name
                          << " (" << hex << recordType
                          << ", index" << dec << index << ")";
#endif
            ...
            case (META_SCALEWINDOWEXT & 0xff):
                {
                    ...
                    //qDebug() <<"WmfParser::ScaleWindowExt :" << widthDenum <<"" << heightDenum;
                }
                break;
            ...
            case (META_PATBLT & 0xff):
                {
                    ...
                    //qDebug() << "patBlt record" << hex << rasterOperation << dec
                    //              << x << y << width << height;
            ...
            case (META_TEXTOUT & 0xff):
                {
                    ...
                    QString strtext = tc->toUnicode(text);

                    stream >> y;
                    stream >> x;

                    m_backend->drawText(mDeviceContext, x, y, strtext);
                }
                break;
            ...
            case (META_SELECTOBJECT & 0xff):
                {
                    ...
                    if ((idx < mNbrObject) && (mObjHandleTab[ idx ] != 0))
                        ...
                        qDebug() << "WmfParser::selectObject : selection of an empty object";
                }
                break;
...
#if DEBUG_RECORDS
                    qDebug() << "text at" << x << y << "length" << stringLength
                                  << ':' << text;
                    //qDebug() << "flags:" << hex << fwOpts << dec;
                    qDebug() << "flags:" << fwOpts;
                    qDebug() << "record length:" << size;
#endif
                    m_backend->drawText(mDeviceContext, x, y, strtext);
                }
                break;
            ...
            case (META_DIBBITBLT & 0xff):
                {
                    ...
                    if (size > 11) {      // DIB image
                        ...
                        qDebug() << "WmfParser::dibBitBlt without image not implemented";
                    }
                }
                break;
            ...
            case (META_DIBCREATEPATTERNBRUSH & 0xff):
                {
                    ...
                    if (addHandle(handle)) {
                        ...
                        if (dibToBmp(bmpSrc, stream, (size - 5) * 2)) {
                            ...
                            qDebug() << "WmfParser::dibCreatePatternBrush : incorrect DIB image";
                        }
                    }
                }
                break;
            ...
            case (META_CREATEFONTINDIRECT & 0xff):
                {
                    ...
                    if (addHandle(handle)) {
                        ...
                        //qDebug() << height << width << weight << property;
                    ...
                    if (addHandle(handle)) {
                        ...
                        if (sty == 2) {
                            if (arg2 < 6)
                                ...
                                //qDebug() << "WmfParser::createBrushIndirect: invalid hatched brush" << arg2;
                            ...
                            if (sty < 9)
                                ...
                                //qDebug() << "WmfParser::createBrushIndirect: invalid brush" << sty;
                                style = Qt::SolidPattern;
                            }
                        }
 ...
 UNSPECIFIED in the Spec: 
    ...
    { &WmfParser::createBitmap, "createBitmap" }, // 110 0xfe
            ...
            default:
                ...
                //qDebug() << "BROKEN WMF file: Record number" << hex << recordType << dec << " index " << index;
                mValid = false;
                break;
            }
...
void WmfParser::createBoundingBox(QDataStream &stream)
{
    ...
    while (recordType) {
        ...
        if (size == 0) {
            qDebug() << "WmfParser: incorrect file!";
            mValid = 0;
            return;
        }
...
#if DEBUG_BBOX
                qDebug() << "setWindowOrg" << windowOrgX << windowOrgY;
#endif
                if (!windowExtIsSet)
                    break;
...
#if DEBUG_BBOX
                qDebug() << "setWindowExt" << windowWidth << windowHeight
...
#if DEBUG_BBOX
                qDebug() << "setViewportOrg" << viewportOrgX << viewportOrgY;
#endif
                orgX = viewportOrgX;
                orgY = viewportOrgY;
                if (viewportExtIsSet) {
...
#if DEBUG_BBOX
                qDebug() << "setViewportExt" << viewportWidth << viewportHeight;
#endif
                orgX = viewportOrgX;
                orgY = viewportOrgY;
                extX = viewportWidth;
...
#if DEBUG_BBOX
            qDebug() << "drawing record: " << (recordType & 0xff);
#endif
...
#if DEBUG_BBOX
        if (isOrgOrExt) {
            qDebug() << "              mBBoxTop = " << mBBoxTop;
            qDebug() << "mBBoxLeft = " << mBBoxLeft << "  mBBoxRight = " << mBBoxRight;
            qDebug() << "           MBBoxBotton = " << mBBoxBottom;
            qDebug() << "Max width,height = " << mMaxWidth << mMaxHeight;
        }
#endif
...
bool WmfParser::addHandle(KoWmfHandle* handle)
{
    ...
    if (idx < mNbrObject) {
        ...
        delete handle;
        mStackOverflow = true;
        qDebug() << "WmfParser::addHandle : stack overflow = broken file !";
        return false;
    }
}
...
void WmfParser::deleteHandle(int idx)
{
    if ((idx < mNbrObject) && (mObjHandleTab[idx] != 0)) {
        ...
        qDebug() << "WmfParser::deletehandle() : bad index number";
    }
}
...
//    if ( !bmp.loadFromData( (const uchar*)bmpHeader, pattern.size(), "BMP" ) ) {
    if (!bmp.loadFromData(pattern, "BMP")) {
        qDebug() << "WmfParser::dibToBmp: invalid bitmap";
        return false;
    } else {
        return true;
    }
