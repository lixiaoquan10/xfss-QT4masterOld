...
void CClientBusiness::recvData(int nMsgType, QByteArray &data)
{
    ...
    switch(type)
    {
        case NCT_ObjectStatus:
        {
            ...
            CCanport* canport = CGlobal::instance()->controller()->canportById(msgObjectStatus->nPanelID);
            if(!canport)
                canport = createUndefinedCanport(CGlobal::instance()->controller(), msgObjectStatus->nPanelID);
