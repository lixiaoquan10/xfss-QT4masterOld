#include "cdevicesfindtablemodel.h"
#include "dm/cdm.h"
...
QVariant CDevicesFindTableModel::getDisplayData(const QModelIndex &index) const
{
    ...
    if(!device)
        return QVariant();
    CDeviceType* devType = CGlobal::instance()->deviceType(device->value(DEVICE_VALUE_TYPE).toInt());
    if(!devType)
...
QVariant CDevicesFindTableModel::getDecorationData(const QModelIndex &index) const
{
    ...
    if(!device)
        return QVariant();
    const CDeviceType* devType = CGlobal::instance()->deviceType(device->value(DEVICE_VALUE_TYPE).toInt());
    if(!devType)
        devType = CGlobal::instance()->deviceType(0);
