...
#define DISTRIBUTION_MAX_LOOP_NUM 6
...
public:
    ...
    CDistribution(CController *controller, const int keyId = 0);
    ~CDistribution();

    static void createTable(CDBSqlite* db);
    static void clearTable(CDBSqlite* db);
    static int loadDistribution(CDBSqlite *db, CController *controller);
    ...
    QString typeStr() const { return value(DISTRIBUTION_VALUE_NAME).toString(); }
    QString descriptionStr() const { return value(DISTRIBUTION_VALUE_DESCRIPTION).toString(); }

    bool setDistributionId(int id) { return setValue(DISTRIBUTION_VALUE_ID, id); }
    int distributionKeyId() const { return value(DISTRIBUTION_VALUE_KEYID).toInt(); }
    int distributionId() const { return value(DISTRIBUTION_VALUE_ID).toInt(); }
    int distributionAddress() const { return value(DISTRIBUTION_VALUE_ADDR).toInt(); }
    int distributionTypeId() const { return value(DISTRIBUTION_VALUE_TYPEID).toInt(); }
    QString distributionProductId() const { return value(DISTRIBUTION_VALUE_DESCRIPTION).toString(); }
