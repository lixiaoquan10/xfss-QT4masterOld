...
bool CLog::addLog(CMsgLog* log)
{
    ...
    if (!query.exec()) {
        ...
        CGlobal::instance()->DlgInformationWindow()->setDescription(QString("<h2><font color=red>%1<br/>%2<br/>%3<br/>%4</font></h2>")
                                                                    .arg(QObject::tr("Log database error, Please contact the maintenance personnel!"))
                                                                    .arg(QObject::tr("Error No:%1").arg(query.lastError().number()))
...
void CLog::openDB(const QString& dbName)
{
    ...
    if (!m_db.open()) {
        ...
        CGlobal::instance()->DlgInformationWindow()->setDescription(QString("<h2><font color=red>%1<br/>%2<br/>%3<br/>%4</font></h2>")
                                                                    .arg(QObject::tr("Log database error, Please contact the maintenance personnel!"))
                                                                    .arg(QObject::tr("Error No:%1").arg(m_db.lastError().number()))
