...
void CCommunicationManager::handleCan1ReciveData()
{
    ...
    if (reciveData.isEmpty())
        return;
    CAN1_reciveData.append(reciveData);
    reciveData.clear();
    if(CAN1_reciveData.count() < 4)
        return;
    int count = 0;
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte1 = static_cast<unsigned char>(CAN1_reciveData.at(0));
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte2 = static_cast<unsigned char>(CAN1_reciveData.at(1));
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte3 = static_cast<unsigned char>(CAN1_reciveData.at(2));
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte4 = static_cast<unsigned char>(CAN1_reciveData.at(3));
    for(int i=0; i<CAN1_reciveData.size(); i++)
    {
        if((dataByte1 == 0x55) && (dataByte2 == 0xaa) && (dataByte3 == 0xcc) && (dataByte4 == 0x33))
        {
            if((i==CAN1_reciveData.size()-1) || ((i<CAN1_reciveData.size()-1) && (static_cast<unsigned char>(CAN1_reciveData.at(i+1)) == 0x55)))
            {
                count = i+1;
                break;
            }
        ...
        {
            CAN1_reciveData.remove(0,1);
        }
    }
    QByteArray truncatedData = CAN1_reciveData.left(count);
    QString  data = "cancard1ReciveData:  " + truncatedData.toHex() + "   " + QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    ...
    if(pSerialPort){
        ...
        PackageTemplate *pack = packAnalyzer.doAnalysis(truncatedData);
        CAN1_reciveData.remove(0,count);
        if(pack)
        {
...
void CCommunicationManager::handleCan2ReciveData()
{
    ...
    CAN2_reciveData.append(reciveData);
    reciveData.clear();
    if(CAN2_reciveData.count() < 4)
        return;
    int count = 0;
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte1 = static_cast<unsigned char>(CAN2_reciveData.at(0));
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte2 = static_cast<unsigned char>(CAN2_reciveData.at(1));
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte3 = static_cast<unsigned char>(CAN2_reciveData.at(2));
    // 提取第一个字节并转换为无符号字符类型
    unsigned char dataByte4 = static_cast<unsigned char>(CAN2_reciveData.at(3));
    for(int i=0; i<CAN2_reciveData.size(); i++)
    {
        if((dataByte1 == 0x55) && (dataByte2 == 0xaa) && (dataByte3 == 0xcc) && (dataByte4 == 0x33))
        {
            if((i==CAN2_reciveData.size()-1) || ((i<CAN2_reciveData.size()-1) && (static_cast<unsigned char>(CAN2_reciveData.at(i+1)) == 0x55)))
            {
                count = i+1;
                break;
            }
        ...
        {
            CAN2_reciveData.remove(0,1);
        }
    }
    QByteArray truncatedData = CAN2_reciveData.left(count);

    QString  data = "cancard2ReciveData:  " + truncatedData.toHex() + "   " + QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz");
    ...
    if(pSerialPort){
        static PackageAnalyzer packAnalyzer;
        PackageTemplate *pack = packAnalyzer.doAnalysis(truncatedData);
        CAN2_reciveData.remove(0,count);
        if(pack)
        {
            int nInfoType = pack->getInfoType();
            QByteArray tmpByteArray = pack->getInfo().toByteArray();
