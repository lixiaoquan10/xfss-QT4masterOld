...
//    sysMenu->addAction(QIcon(":/images/Close.png"), "弹出U盘", this, SLOT(slotOutUSB()));
    ...
    sysMenu->addAction(QIcon(":/images/UserConfig.png"), "刷新串口", this, SLOT(slotRefreshUSB()));
    sysMenu->addSeparator();
...
void CTopToolBar::slotRefreshUSB()
{
    qDebug() << "CTopToolBar::slotRefreshUSB" << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    CGlobal::instance()->m_HaveDialog = true;
    // 创建一个QFont对象并设置字体属性
    QFont font;
    font.setPointSize(12); // 设置字体大小为12像素
    ...
    messageBox.setStyleSheet(styleSheet);
    messageBox.setIcon(QMessageBox::NoIcon);
    messageBox.setWindowTitle("刷新串口");
    messageBox.setStandardButtons(QMessageBox::Yes | QMessageBox::Cancel);
    messageBox.setDefaultButton(QMessageBox::Cancel);
    QString log;
    log = "是否刷新串口？";
    ...
    if(ret == QMessageBox::Yes)
    {
        QProcess::execute("sudo udevadm control --reload-rules");
        QProcess::execute("sudo udevadm trigger");
    }
    CGlobal::instance()->m_HaveDialog = false;
    m_btnSystemTool->setChecked(true);
}
