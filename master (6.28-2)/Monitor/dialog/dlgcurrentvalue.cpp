#include "dlgcurrentvalue.h"
#include "ui_dlgcurrentvalue.h"
...
dlgCurrentValue::dlgCurrentValue(const CLoop *loop, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::dlgCurrentValue),
    m_loop(loop), m_x(loop->deviceCount()), m_needUpdate(false)
{
    ui->setupUi(this);
    this->setWindowTitle(tr("View current value"));
...
dlgCurrentValue::~dlgCurrentValue()
{
    connectLoopDevice(m_loop, false);
    delete ui;
}

void dlgCurrentValue::initCustomPlot(QCustomPlot *customPlot)
{
...
void dlgCurrentValue::connectLoopDevice(const CLoop *loop, bool enable)
{
...
void dlgCurrentValue::updateCurrentValue()
{
    int count = m_loop->deviceCount();
    QVector<double> data(count);
    for(int i = 0; i < count; ++i)
        data[i] = m_loop->device(i)->getStatus(OBJS_CurrentValue);
    m_bars->setData(m_x, data);
    ui->customPlot->replot();
    m_needUpdate = false;
}

void dlgCurrentValue::timerUpdate()
{
    static int n = 3;
    n++;
    if(n >= 3)
    {
        ...
        CGlobal::instance()->ClientBusiness()->exeCommand(NCT_GetCurrentValue, &msgDeviceInfo);
    }
    if(m_needUpdate)
        updateCurrentValue();
}

void dlgCurrentValue::loopDeviceStatusChanged(short status, unsigned char oldValue, unsigned char newValue, unsigned int time)
{
    Q_UNUSED(oldValue);
    Q_UNUSED(newValue);
    Q_UNUSED(time);
    if(status == OBJS_CurrentValue)
        m_needUpdate = true;
}
