...
//                                        serverDeviceRealtimeData(distribution);
                        ...
                        if(distributionAddress == 0x3f)
                        {
                            ...
                            {
                                CGlobal::instance()->m_version_can1can2 = QString::number(softwareNumber) + "V" + QString::number(softwareVersion);
                            }
                            else if(msgObjectStatus->nCanportAdd == 5 || msgObjectStatus->nCanportAdd == 6)
                            {
                                CGlobal::instance()->m_version_can3can4 = QString::number(softwareNumber) + "V" + QString::number(softwareVersion);
                            }
...
//            serverManualLaunchUpload();
        ...
        case NCT_ControllerParam:
        {
            if(data.count() < 4)
                return;
            CController* controller = CGlobal::instance()->controller();
            controller->setControllerValue(CONTROLLER_VALUE_MAINPOWER,(data.at(0) + data.at(1)*256)/100);
            controller->setControllerValue(CONTROLLER_VALUE_BATPOWER,(data.at(2) + data.at(3)*256)/100);
//            CGlobal::instance()->m_ControllerMainpower = (data.at(0) + data.at(1)*256)/100;    //主电电压
//            CGlobal::instance()->m_ControllerBatpower = (data.at(2) + data.at(3)*256)/100;     //备电电压
            break;
        }
...
//按页面启动应急
void CClientBusiness::layoutPageEmergency(int firePointID)
{
    int direction = 0;
