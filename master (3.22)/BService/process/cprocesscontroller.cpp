...
//        double Ledstatus_mainpower = (data.at(5) + data.at(6)*256)/100;    //主电电压
//        double Ledstatus_batpower = (data.at(7) + data.at(8)*256)/100;     //备电电压
        QByteArray byte = data.mid(4, 4); // 从索引4 开始提取长度为 4 的字节
        CGlobal::instance()->DealEvent(NCT_ControllerParam, byte);
        if(Ledstatus_key != 0)
        {
            CMsgNull msgNull;
            //无源联动输入
...
void CProcessController::SystemSet(CMsgObjectStatus *msgObjectStatus)
{
    ...
    m_monthDuration = msgObjectStatus->ndata.at(6);
...
void CProcessController::yearCheck()
{
    ...
    //    addLog(CPrint::Other, controllerStr() + "年检开始");
}
//月检开始
void CProcessController::monthCheck()
{
    ...
    //    addLog(CPrint::Other, controllerStr() + "月检开始");
}
//结束年检
