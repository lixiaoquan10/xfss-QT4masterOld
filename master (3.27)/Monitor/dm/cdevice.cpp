...
bool CDevice::setDeviceValue(QString name, QVariant value)
{
    ...
    {
        if(m_loop && m_loop->loopValue(OBJECT_VALUE_ONLINE).toInt() == 1)
        {
            if(value == 1 && deviceValue(OBJECT_VALUE_ONLINE).toInt() != 1)
                m_loop->adjustedDeviceOnlineNumber(deviceTypeID(), 1);
            else if(value == 0 && deviceValue(OBJECT_VALUE_ONLINE).toInt() == 1)
                m_loop->adjustedDeviceOnlineNumber(deviceTypeID(), -1);
        }
    }
    QVariant old;
