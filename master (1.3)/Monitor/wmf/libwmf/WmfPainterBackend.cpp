...
WmfPainterBackend::WmfPainterBackend(QPainter *painter, const QSizeF &outputSize)
    ...
    , mSaveCount(0)
{
    if (painter) mTarget = painter->device();
...
#if DEBUG_WMFPAINT
    qDebug() << "Using QPainter: " << mPainter->pen() << mPainter->brush()
...
void WmfPainterBackend::restore()
{
    if (mSaveCount > 0) {
        ...
        qDebug() << "restore(): try to restore painter without save";
    }
...
void WmfPainterBackend::recalculateWorldTransform()
{
...
#if DEBUG_WMFPAINT
        qDebug() << "Scale for Window -> Viewport"
...
#if DEBUG_WMFPAINT
        qDebug() << "Only one of Window or Viewport set: scale window -> viewport = 1";
#endif
    }
    ...
    if (flip) {
        //qDebug() << "Flipping round midpoint" << midpointX << midpointY << scaleX << scaleY;
    ...
    //qDebug() << "After window viewport calculation" << mWorldTransform;
    ...
    //qDebug() << "Output transform" << mOutputTransform;
    //qDebug() << "Total  transform" << newMatrix;
...
void WmfPainterBackend::drawPolyPolygon(WmfDeviceContext &context, QList<QPolygon>& listPa)
{
    ...
    // fill polygons
    if (brush != Qt::NoBrush) {
        //qDebug() << "Filling polygon with " << brush;
...
{
#if DEBUG_WMFPAINT
    qDebug() << x << y << width << height << hex << rasterOperation << dec;
#endif
...
void WmfPainterBackend::drawText(WmfDeviceContext &context, int x, int y, const QString& text)
{
...
#if DEBUG_WMFPAINT
        qDebug() << "Using current position:" << x << y;
#endif
    }
...
#if DEBUG_WMFPAINT
    qDebug() << "font = " << mPainter->font() << " pointSize = " << mPainter->font().pointSize()
    ...
    //qDebug() << "fontheight:" << context.height << "height:" << height << "y" << y;
...
void WmfPainterBackend::updateFromDeviceContext(WmfDeviceContext &context)
{
...
#if DEBUG_WMFPAINT
        qDebug() << "*** Setting fill brush to" << context.brush;
#endif
    }
    // FIXME: context.image
...
#if DEBUG_WMFPAINT
        qDebug() << "*** Setting font to" << context.font;
#endif
    }
    if (context.changedItems & DCPalette) {
        // NYI
#if DEBUG_WMFPAINT
        qDebug() << "*** Setting palette (NYI)";
#endif
    }
...
#if DEBUG_WMFPAINT
        qDebug() << "*** Setting background text color to" << context.backgroundColor;
#endif
    }
    //----------------------------------------------------------------
    ...
    if (context.changedItems & DCBgMixMode) {
        ...
        mPainter->setBackgroundMode(context.bgMixMode == TRANSPARENT ? Qt::TransparentMode
                                                                     : Qt::OpaqueMode);
#if DEBUG_WMFPAINT
        qDebug() << "*** Setting background mode to" << context.bgMixMode;
#endif
    }
...
#if DEBUG_WMFPAINT
        qDebug() << "*** Setting composition mode to" << context.rop;
#endif
    }
