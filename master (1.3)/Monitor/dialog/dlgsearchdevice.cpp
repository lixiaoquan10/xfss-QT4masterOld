    ...
    ui(new Ui::dlgSearchDevice)
{
    ...
    CController * controller = CGlobal::instance()->ClientBusiness()->currentPanel();
    if(controller){
        for (int i=0; i<controller->distributionCount(); ++i)
        {
            CDistribution* distribution = controller->distribution(i);
            ui->comboBox->addItem(distribution->value(DISTRIBUTION_VALUE_NAME).toString(), distribution->distributionId());
        }
    }
}
...
void dlgSearchDevice::on_comboBox_currentIndexChanged(int index)
{
    ...
    CDistribution* distribution = controller->distributionById(nID);
    ui->comboBox_2->clear();
    ui->comboBox_2->addItem(tr("All Loop"), 0);
    if(distribution)
    {
        for (int i=0; i<distribution->loopCount(); ++i)
        {
            ...
            if(loop)
                ui->comboBox_2->addItem(loop->value(LOOP_VALUE_NAME).toString(), loop->loopId());
        }
    }
