...
{
//    db->query("CREATE TABLE IF NOT EXISTS loop ("
//                  "id INTEGER PRIMARY KEY AUTOINCREMENT,"
//                  "keyid INTEGER,"
//                  "parent_keyid INTEGER,"
//                  "loop_id INTEGER,"
//                  "loop_name VARCHAR(255),"
//                  "loop_description VARCHAR(255)"
//              ");").exec();
//    db->query("CREATE UNIQUE INDEX IF NOT EXISTS idx_loop ON loop ("
//                  "parent_keyid ASC,"
//                  "loop_id ASC"
//              ");").exec();
...
//{
//    QSqlQuery query = db->query(QString("SELECT keyid, loop_id, loop_name, loop_description "
//                                        "FROM loop WHERE parent_keyid = %1")
//                                .arg(distribution->keyId()));
//    QSqlRecord record = query.record();
//    int filedKeyId = record.indexOf("keyid");
//    int filedLoopId = record.indexOf("loop_id");
//    int filedLoopName = record.indexOf("loop_name");
//    int filedLoopDescription = record.indexOf("loop_description");
//    while(query.next())
//    {
//        CLoop* loop = new CLoop(query.value(filedKeyId).toInt());
//        loop->setValue(LOOP_VALUE_ID, query.value(filedLoopId).toInt());
//        loop->setValue(LOOP_VALUE_NAME, query.value(filedLoopName));
//        loop->setValue(LOOP_VALUE_DESCRIPTION, query.value(filedLoopDescription));
////        loop->setNone();
...
//    int filedLoopName = record.indexOf("loop_name");
    ...
    while(query.next())
    {
        ...
        loop->setValue(LOOP_VALUE_ID, query.value(filedKeyId).toInt());
        loop->setValue(LOOP_VALUE_NUMBER, query.value(fieldNumberId).toInt());
        loop->setValue(LOOP_VALUE_ADDR, query.value(fieldLoopAdd).toInt());
        loop->setValue(LOOP_VALUE_NAME, tr("回路%1").arg(query.value(fieldLoopAdd).toInt()));
        loop->setValue(LOOP_VALUE_REMARK, query.value(fieldRemark));
        loop->setValue(LOOP_VALUE_DISTRIBUTIONADDR, distribution->distributionAddress());
        distribution->addLoop(loop);    
        if(CDevice::loadDevice(db, loop) != 0)
            return -1;
...
bool CLoop::addDevice(CDevice *device)
{
    ...
    if(device->getStatus(OBJS_Online))
...
QVariant CLoop::afterGetValue(QString name, QVariant value) const
{
    if(name == OBJECT_VALUE_DISPLAY_NAME)
        return m_value.value(LOOP_VALUE_NAME);
    else if(name == OBJECT_VALUE_ADDRESS)
        return m_value.value(LOOP_VALUE_ADDR);
    else if(name == OBJECT_VALUE_DESCRIPTION)
        return m_value.value(LOOP_VALUE_DESCRIPTION);
...
void CLoop::initLoop()
{
    m_distribution = NULL;
    m_maxDeviceId = 0;
    setValue(OBJECT_VALUE_PIXMAP, ":/images/loop.png");
    setValue(LOOP_VALUE_NAME, "回路");
...
int CLoop::saveLoop(CDBSqlite *db)
{
    ...
    query.bindValue(":Loop", value(LOOP_VALUE_ADDR));
    query.bindValue(":Remark", " ");
    qDebug() << "loop==========" << "LoopID:"<< loopkeyId << "distributionID:"<< distributionkeyId << "loopAdd:"<< value(LOOP_VALUE_ADDR).toInt();
    if(!query.exec())
        return -1;
    if(saveDevice(db) != 0)
        return -1;
...
int CLoop::saveDevice(CDBSqlite *db)
{
    for(int i = 0; i < m_devices.size(); ++i)
    {
        if(!CGlobal::instance()->m_isDelUnregLampFlag || (CGlobal::instance()->m_isDelUnregLampFlag &&
               (m_devices.at(i)->value(DEVICE_VALUE_ISUSED) == 2 || m_devices.at(i)->value(DEVICE_VALUE_ISUSED) == 1)))
        {
