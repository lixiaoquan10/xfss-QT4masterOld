...
CCanport::CCanport(CProject *project, const int keyId) :
    CObject(keyId), m_onlineDistributionCount(0), m_onlinePowerCount(0), m_deviceOnlineTotal(0), m_onlineTotalCount(0)
{
    initCanport();
    int n = project->maxCanportId() + 1;
    setCanportId(n);
    project->addCanport(this);
...
int CCanport::loadCanport(CDBSqlite *db, CProject *project)
{
    CCanport* canport = new CCanport(63);
    canport->setCanportValue(CANPORT_VALUE_ID,63);
    canport->setCanportValue(CANPORT_VALUE_PORT, 1);
    canport->setCanportValue(CANPORT_VALUE_ADDRESS, 63);
    canport->setCanportValue(CANPORT_VALUE_AREA, "");
    canport->setCanportValue(CANPORT_VALUE_LOCATION, "");
    canport->setCanportValue(CANPORT_VALUE_NAME, "控制器");
    canport->setNone();
    project->addCanport(canport);
//    canport->setStatus(OBJS_Online,1);
...
void CCanport::afterSetValue(QString &name, QVariant& oldValue, QVariant& newValue)
{
    ...
    {
        if(m_project)
            m_project->calcMaxCanportId(newValue.toInt());
    }
...
void CCanport::initCanport()
{
    ...
    setCanportValue(OBJECT_VALUE_PIXMAP, ":/images/controller.png");
    setCanportValue(CANPORT_VALUE_NAME, "控制器");
