...
void CDesignSplashView::setPrimitiveItem(int layerID)
{
    ...
    while(query.next())
    {
        ...
        CDistribution* distribution = canport->distributionByAddress(query.value(10).toInt());
        if(!distribution)
            continue;
...
//        int LampInfoID = query.value(fieldLampInfoID).toInt();
    ...
    if(CGlobal::instance()->m_isEmergency)
    {
        ...
        {
            CDistribution* distribution1 = canport->distributionByAddress(queryConnection.value(6).toInt());
            if(!distribution1)
                continue;
            ...
            CDistribution* distribution2 = canport->distributionByAddress(queryConnection.value(9).toInt());
            if(!distribution2)
                continue;
